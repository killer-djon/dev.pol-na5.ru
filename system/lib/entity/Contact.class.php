 ...
 * @version SVN: $Id: Contact.class.php 10248 2011-12-05 12:47:29Z alexmuz $
 */
class Contact
{
...
HTML;
	...
	public static function getSubscribeLink($contact_id, $fix = false, $strong = true)
	{
	    $strong = defined('STRONG_AUTH') && STRONG_AUTH;
		$contacts_model = new ContactsModel();
	...
	public static function searchByFields($info, $sort = false, $limit = false, $where = "", $list_id = false, $list_title = false, $type = 'smart')
	{
		...
		foreach ($info as $kk => $row) {
			if ($where) {
				$where .= (isset($row['link']) && isset($links[$row['link']]) ? $links[$row['link']] : " AND ");
			}
			if ($type == 'advanced') {
