...
function GetExtraParametrs( $productID ){
	...
	if(count($ProductIDs)){
		...
		while ($_Row = db_fetch_assoc($Result)) {
			...
			LanguagesManager::ml_fillFields(PRODUCT_OPTIONS_TABLE, $_Row);
			$b=null;
			// && !LanguagesManager::ml_isEmpty('option_value', $_Row['option_value'])
			if (($_Row['option_type']==0 || $_Row['option_type']==NULL) && !empty($_Row['option_value']) ){
			
				//$_Row['option_value'] = preg_replace( array('/(,+)/', '/([^\d]+)/') , array('.', 'x'), $_Row['option_value']);	
				if( $_Row['optionID'] == 63 )
					$_Row['option_value'] = preg_replace('/(,+)/', '.', $_Row['option_value']);
					
				if( $_Row['optionID'] == 62 )
					$_Row['option_value'] = preg_replace('/([^\d]+)/', ' x ', $_Row['option_value']);
