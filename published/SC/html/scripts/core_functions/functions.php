...
function xStripSlashesGPC($_data) {
	if (version_compare('5.4', PHP_VERSION, '>') && function_exists('get_magic_quotes_gpc') && @get_magic_quotes_gpc()) {
		$_data = is_array($_data)? array_map(__FUNCTION__,$_data) : stripslashes($_data);
	}
	return $_data;
}
...
function ss_mail($email, $subject, $text, $is_html = true,$headers = array()){
	...
	if(false && isset($headers['From']) && $headers['From'] != CONF_GENERAL_EMAIL ) {
		$from = $headers['From'];
		$blacklist = array();
		$blacklist['text'] = array( // list of deprecated messages
			'http',
			'www.',
		// add custom words here
		);
		$blacklist['subject'] = array( // list of deprecated messages
			'http',
			'www.',
		// add custom words here
		);
		$blacklist['from'] = array( // list of deprecated emails
			'domain.net',
		// add custom words here
		);
		foreach($blacklist as $item => $words) {
	...
	$mailer = new SSMailer();
	$mailer->From		 = isset($headers['From'])?$headers['From']:CONF_GENERAL_EMAIL;
	$mailer->Sender		 = isset($headers['Sender'])?$headers['Sender']:CONF_GENERAL_EMAIL;
	$mailer->FromName	 = isset($headers['FromName'])?$headers['FromName']:CONF_SHOP_NAME;
