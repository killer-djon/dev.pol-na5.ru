...
function _sendOrderNotifycationToAdmin( $orderID, &$smarty_mail, $tax )
{
	...
	if (false) {
	    ...
	    $headers['FromName'] = $order['customer_firstname'].' '.$order['customer_lastname'];
	}

	$res = ss_mail( CONF_ORDERS_EMAIL, translate('ordr_id')." ".$order['orderID_view'],$html, true,$headers);
...
function ordCalculateOrderTax(&$order, $order_content)
{
	...
	foreach ($order_content as $_item)	{
		$order['tax'] += ($order['currency_value']*$_item['Price']*$_item['Quantity'])/100*/*(100-$order['order_discount(_percent)'])/100**/$_item['tax'];
	}
