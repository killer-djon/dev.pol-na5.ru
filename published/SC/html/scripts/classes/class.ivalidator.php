...
class IValidator
{
	...
	{
		if(!$_Code || empty($_SESSION[$this->SPrefix.'IVAL'])) {
	...
	function create_captcha($data = '', $img_path = '', $img_url = '', $font_path = '')
	{
		...
		for ($i = 0; $i < strlen($word); $i++) {
			if ($use_font) {
				...
				$angle	= rand(-15, 15);
				$res = imagettftext($im, $font_size, $angle+rand(-$angle_offset,$angle_offset), $x+rand(-$offset,$offset), $y+rand(-$offset,$offset), $shadow_color, $font_path, substr($word, $i, 1));
