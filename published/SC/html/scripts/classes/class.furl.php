			...
			if(isset($get['refid'])){
				...
				switch ($get['ukey']){
					...
					case CONF_ARTCLE_URL:
						$request .= CONF_ARTCLE_URL.'/'.(isset($get['rss'])?$get['rss'].'/':'').(isset($get['CategorySlug'])?$get['CategorySlug'].'/':'').(isset($get['postSlug'])?$get['postSlug'].'/':'');
							unset($get['rss']);
							unset($get['CategoryID']);
							unset($get['CategorySlug']);
							unset($get['ukey']);
							unset($get['postID']);
							unset($get['postSlug']);
							unset($get['did']);
					...
					default:
						switch ($part_value){
							...
							case 'transaction':
								...
								continue;	
							case CONF_ARTCLE_URL:
								$this->__path_settings['ukey'] = CONF_ARTCLE_URL;
								if(strpos($path_parts[0], 'rss') === 0){
									$this->__path_settings['get'] .= '&rss=1';
								}else{
									$this->__path_settings['get'] .= '&CategorySlug='.$path_parts[0];
								}
								if(strpos($path_parts[1], 'offset') === 0){
									$this->__path_settings['get'] .= '&offset='.intval(substr($path_parts[1], 6));
								}elseif(strpos($path_parts[1], 'all') === 0){
									$this->__path_settings['get'] .= '&show_all=1';
								}else{
									$this->__path_settings['get'] .= '&postSlug='.$path_parts[1];
								}
								continue;
							default:
