...
class GoogleSitemap extends ActionsController
{
	...
	CONF_ARTCLE_URL	=>array('priority'=>0.5,'changefreq'=>null),
	);
	function update()
	{
...
<sitemapindex xmlns="http://www.sitemaps.org/schemas/sitemap/0.9"
	...
	xsi:schemaLocation="http://www.sitemaps.org/schemas/sitemap/0.9
		...
		switch($name){
			...
			case CONF_ARTCLE_URL:
				$exportData->setHeaders(array('loc','priority','changefreq'));
				$exportData->sqlWhereClause = 't1.ArticleEnable = 1';
				$exportData->sqlGroupClause = '';
				$exportData->sqlQuery = 'SELECT t2.`CategorySlug` as `CategorySlug`, t1.`ArticleID` as `ArticleID`, t1.`ArticleCategoryID` as `CategoryID`, t1.`ArticleSlug` as `ArticleSlug` FROM ?#ARTICLES_CATEGORY_TABLE t2 RIGHT JOIN ?#ARTICLES_TABLE t1 ON(t2.`CategoryID` = t1.`ArticleCategoryID`)';
				...
				$rows = array();
				if(!$row[\'CategorySlug\']){
					$row[\'CategorySlug\'] =CONF_ARTCLE_ROOT_URL;
				}
				$rows[] = array(\'url\' => $_POST[\'base_url\'].str_replace($___base_path,\'\',set_query("?ukey=".CONF_ARTCLE_URL."&CategoryID={$row[\'CategoryID\']}&CategorySlug={$row[\'CategorySlug\']}&postID={$row[\'ArticleID\']}&postSlug={$row[\'ArticleSlug\']}".(MOD_REWRITE_SUPPORT?"&furl_enable=1":""))),
								\'priority\'=>0.8,
								\'changefreq\'=>\'daily\');
				return $rows;');
				break;
