...
class YandexMarket extends Module {
	...
	function _exportBegin( $f )
	{
		...
		fputs( $f, "		<yml_catalog date=\"".date("Y-m-d H:i",Time::serverTimeToTime(time()))."\">\n" );
		fputs( $f, "			<shop>\n" );
	...
	function _exportProducts( $f, $rate, $export_product_name, &$_ProductIDs )
	{
		...
		for (; $iter<$proCount;$iter+=$chunk_size){
			...
			while ($product = db_fetch_row($q))
			{
				...
				fputs( $f, "						<url>".str_replace('&','&amp;',set_query('ukey=product'.(MOD_REWRITE_SUPPORT?'&furl_enable=1':'').'&product_slug='.$product['slug'].'&productID='.$product['productID'].'&from=ya',$store_url))."</url>\n" );
