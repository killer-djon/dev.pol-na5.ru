<?php
	...
	if(!is_null($searchstr) && $searchstr){
		...
		while ($row = $DBRes->fetchAssoc()){
			$locals[$row['id']] = array(
				'id' => $row['id'],
				'id_search' => preg_replace("/({$pattern})/ui","<span style=\"color:orange;font-weight: bolder;\">$1</span>",$row['id']),
				'value' => $row['value'],
				'defvalue' => preg_replace("/({$pattern})/ui","<span style=\"color:orange;font-weight: bolder;\">$1</span>",htmlspecialchars($row['alt_value'], ENT_QUOTES)),
			);
		}
	}
