 ...
 * @version 1.6
 * @link http://www.qiwi.ru/
 */
class QiwiSOAP extends PaymentModule
{
	var $type = PAYMTD_TYPE_ONLINE;
	var $default_logo = './images_common/payment-icons/qiwi.gif';
	var $language = 'rus';
				...
				if($orderID && ($order = _getOrderById($orderID))){
					if($result = $this->checkBill($orderID)){
						...
						switch(intval($result->status)){
							case 60:{
								$amount = PaymentModule::_convertCurrency($order['order_amount'],0,$this->_getSettingValue('CONF_QIWI_CURRENCY'));
								if($amount != floatval($result->amount)){
						...
						$this->_log(LOGTYPE_ERROR,'checkBill failed for orderID='.$orderID);
					}
				}else{
	...
	public function payment_form_html()
	{
		...
		$title = QIWI_TXT_CUSTOMER_PHONE.' +7';
		$storage = Storage::getInstance(__CLASS__);
		$customer_phone = $storage->getData('customer_phone');
		if(!$customer_phone && ($field_id = $this->_getSettingValue('CONF_QIWI_CUSTOMER_PHONE'))){
...
HTML;
	...
	function payment_process($order)
	{
		...
		if($customer_phone) {
		    ...
		    $storage->setData('customer_phone', $customer_phone);
		}
	...
	public function after_processing_html($orderID, $active = true)
	{
		...
		$customer_phone = preg_replace('@[\D]+@','',$customer_phone);
		$customer_phone = preg_replace('@^\+7@','',$customer_phone);
		...
		$phone_description = QIWI_TXT_CUSTOMER_PHONE.' +7';
...
HTML;
	...
	private function getPassword($orderID)
	{
		...
		if(setlocale(LC_CTYPE, 'ru_RU.CP-1251', 'ru_RU.CP1251', 'ru_RU.win','ru_RU.1251','Russian_Russia.1251', 'Russian_Russia.CP-1251', 'Russian_Russia.CP1251', 'Russian_Russia.win') === false) {
			$this->_log(LOGTYPE_ERROR,__METHOD__."\t{$orderID}\setLocale failed");
		}
	...
	public function updateBill($login,$password,$txn,$status)
	{
		...
		$args = func_get_args();
		$this->_log(LOGTYPE_DEBUG,__METHOD__.' '.var_export($args,true));
		$register = Register::getInstance();
		...
		if($login == $this->_getSettingValue('CONF_QIWI_LOGIN')){
			...
			$pattern = preg_replace($cleanup_pattern,'\\\\$1',$string);
			$pattern = "@^{$prefix}@";
			if(preg_match($pattern,$txn)){
				...
				if($order){
					...
					if($password == $system_password){
						...
						$this->_log(LOGTYPE_DEBUG,'get invalid password '.$password.' but expect '.$system_password.' for order '.$orderID.' - '.$txn.' prefix: '.$prefix);
						$result->updateBillResult = 150;
					}
				}else{
					$this->_log(LOGTYPE_ERROR,'Order '.$orderID.' not found. txn:'.$txn.' prefix: '.$prefix);
					$result->updateBillResult = 210;
				}
			}else{
				$this->_log(LOGTYPE_ERROR,'Invalid txn: '.$txn.' prefix: '.$prefix.' pattern: '.$pattern);
				$result->updateBillResult = 210;
			}

		}else{
			$this->_log(LOGTYPE_ERROR,'Invalid login: '.$txn.' prefix: '.$prefix.' pattern: '.$pattern);
