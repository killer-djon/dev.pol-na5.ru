...
class LiqPay extends PaymentModule
{
	...
	var $default_logo = './images_common/payment-icons/liqpay.gif';
	var $language = 'rus';
	...
	function _initSettingFields(){
		...
		$this->SettingsFields['CONF_LIQPAY_ORDER_PREFIX'] = array(
			'settings_value' 			=> '',
			'settings_title' 			=> LIQPAY_CFG_ORDER_PREFIX_TTL,
			'settings_description' 		=> LIQPAY_CFG_ORDER_PREFIX_DSCR,
			'settings_html_function' 	=> 'setting_TEXT_BOX(0,',
			'sort_order' 				=> 1,
		);

		$this->SettingsFields['CONF_LIQPAY_BUGFIX'] = array(
			'settings_value' 			=> '',
			'settings_title' 			=> LIQPAY_CFG_BUGFIX_TTL,
			'settings_description' 		=> LIQPAY_CFG_BUGFIX_DSCR,
	...
	function after_processing_html( $orderID, $active = true )
	{
		...
		$description	 = htmlentities('Оплата заказа '.$order['orderID_view'],ENT_QUOTES,'utf-8');
		...
		$prefix = $this->_getSettingValue('CONF_LIQPAY_ORDER_PREFIX');
		$suffix = '';
		if($this->_getSettingValue('CONF_LIQPAY_BUGFIX')) {
...
<request>      
		...
		<order_id>{$prefix}{$orderID}{$suffix}</order_id>
...
HTML;
	...
	function transactionResultHandler($transaction_result = '',$message = '',$source = 'frontend')
	{
		...
		if($xml_decoded && ($parsed_xml = simplexml_load_string($xml_decoded))) {
			...
			$orderID = $parsed_xml->order_id;
			if($prefix = $this->_getSettingValue('CONF_LIQPAY_ORDER_PREFIX')) {
				...
				$prefix = preg_replace($cleanup_pattern,'\\\\$1',$prefix);
				$orderID = preg_replace("@^{$prefix}@",'',$orderID);
			}
			if($this->_getSettingValue('CONF_LIQPAY_BUGFIX')) {
				$orderID = preg_replace("@_[\d]{4}$@",'',$orderID);
			}
