 ...
 * @link http://www.assist.ru/files/manual_new.pdf
...
class CAssist extends PaymentModule {
	...
	var $language = 'rus';
	var $default_logo = './images_common/payment-icons/assist.gif';
	...
	function _initVars(){
		...
		$this->Settings = array(
				...
				"CONF_PAYMENTMODULE_ASSIST_VERSION",
				"CONF_PAYMENTMODULE_ASSIST_GATE",
		);
	}
	...
	function _initSettingFields(){
		...
		$this->SettingsFields['CONF_PAYMENTMODULE_ASSIST_VERSION'] = array(
			'settings_value' 			=> 'old', 
			'settings_title' 			=> 'Версия подключения к платежной системе', 
			'settings_description'	 	=> 'Выберите версию подключения', 
			'settings_html_function' 	=> 'setting_SELECT_BOX(CAssist::_getVersions(),',
			'sort_order' 				=> 3,
		);
		
		$this->SettingsFields['CONF_PAYMENTMODULE_ASSIST_GATE'] = array(
			'settings_value' 			=> '', 
			'settings_title' 			=> 'Домен', 
			'settings_description' 		=> 'Введите число из домена подключения (https://payments<u>123</u>.paysecure.ru).<br>Используется в новом протоколе подключения.', 
			'settings_html_function' 	=> 'setting_TEXT_BOX(0,', 
			'sort_order' 				=> 1,
		);
	}
	...
	function after_processing_html( $orderID )
	{
		...
		$version = $this->_getSettingValue('CONF_PAYMENTMODULE_ASSIST_VERSION');
		$versions = array_keys($this->url);
...
<table width='100%'>
	<tr>
		...
		<FORM NAME="form1" ACTION="{$this->url[$version]}" METHOD="POST"{$extra_form}>
...
HTML;
			...
			$url = sprintf($this->url[$version],$this->_getSettingValue('CONF_PAYMENTMODULE_ASSIST_GATE'));
			$res .= <<<HTML
			<table width='100%'>
				<tr>
					<td align='center'>
...
HTML;
	...
	static function _getVersions(){
		return array(
			array('title'=>'Старый (http://www.assist.ru/files/manual_new.pdf)','value'=>'old'),
			array('title'=>'Новый (http://www.assist.ru/files/TechNEW.doc)','value'=>'new'),
		);
	}
