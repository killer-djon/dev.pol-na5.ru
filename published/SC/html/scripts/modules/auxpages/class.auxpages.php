...
class AuxAdministrationController extends ActionsController {
 ...
 global $smarty;
  ...
  $AuxID = $moduleEntry->auxpgAddAuxPage( $_POST,$_POST, $_POST, $_POST, $_POST, isset($_POST['aux_page_enabled'])?1:0, $max_priority,$_POST['aux_page_slug'] );
 ...
 $moduleEntry->addAuxPageNameLocal($AuxID, $_POST);
  ...
  $moduleEntry->auxpgUpdateAuxPage( $_GET['edit'], $_POST, 	$_POST, $_POST, $_POST, $_POST, isset($_POST['aux_page_enabled'])?1:0, $_POST['aux_page_slug']);
...
class AuxPages extends ComponentModule {
  ...
  $smarty->assign("page_name",	$AuxInfo["aux_page_name"] );
 
  $smarty->assign("page_meta_tags", $meta_tags );
  
  $smarty->assign('aux_page_intro', $AuxInfo['aux_page_intro']);
 ...
 function auxpgGetAllPageAttributes(){
  ...
  function auxpgUpdateAuxPage( $aux_page_ID, $aux_page_name, $aux_page_intro, $aux_page_text, $meta_keywords, $meta_description, $aux_page_enabled,$aux_page_slug ){
  ...
  $intro_inj = LanguagesManager::sql_prepareFields('aux_page_intro', $aux_page_intro);
 
  foreach ($intro_inj['fields'] as $field) $fields.=$field.'=?,';
  ...
  db_phquery_array($sql,$name_inj['values'], $intro_inj['values'],$text_inj['values'],$mkeywords_inj['values'],$mdescription_inj['values'],$aux_page_enabled, $aux_page_slug, $aux_page_ID); 
  ...
  function auxpgAddAuxPage($aux_page_name, $aux_page_intro,  $aux_page_text, $meta_keywords, $meta_description, $aux_page_enabled, $aux_page_priority,$aux_page_slug  ){
 
 
 
  $name_inj = LanguagesManager::sql_prepareFields('aux_page_name', $aux_page_name, true);
 
  $intro_inj = LanguagesManager::sql_prepareFields('aux_page_intro', $aux_page_intro, true);
  ...
  $fields=$name_inj['fields_list'].','.$text_inj['fields_list'].','. $intro_inj['fields_list'].',';
 
  $fields.=$mkeywords_inj['fields_list'].','.$mdescription_inj['fields_list'];
 
  $values_place=str_repeat('?,',
 
  count($name_inj['values'])+count($text_inj['values'])+count($intro_inj['values'])+
  ...
  db_phquery_array($sql,$name_inj['values'], $intro_inj['values'],$text_inj['values'],$mkeywords_inj['values'],$mdescription_inj['values'],$aux_page_enabled, $aux_page_priority,$aux_page_slug);
