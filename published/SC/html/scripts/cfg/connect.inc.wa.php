...
if(MOD_REWRITE_SUPPORT&&false)
{
	if(true){
		...
		define('URL_ARTICLES_PICTURES', URL_ROOT.'/articles_pictures');
	}else{
		...
		define('URL_ARTICLES_PICTURES', WBS_INSTALL_PATH.'/articles_pictures');
	}
}else{
	define('URL_PUBDATA_ROOT', str_replace('//','/',WBS_INSTALL_PATH.'/published/publicdata'));
	$DB_KEY=SystemSettings::get('DB_KEY');
	foreach(array('products_pictures','images','themes','rss','articles_pictures') as $fld)
	{
...
if(!defined('DIR_PUBLICDATA_SC')){
	...
	define('DIR_ARTICLES_PICTURES', DIR_PUBLICDATA_SC.'/articles_pictures');
