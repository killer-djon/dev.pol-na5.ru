...
class AddressEditor extends ActionsController
{
	...
	function save()
	{
		...
		$addressID = max(0,intval($this->getData('addressID')));
		$customerEntry = Customer::getAuthedInstance();
		if($addressID){
		    $addressEntry->loadByID($addressID);
			if($addressEntry->customerID != $customerEntry->customerID) {
			    Message::raiseMessageRedirectSQ(MSG_ERROR, '', 'err_forbidden', '' , array('Data'=>$this->getData()));
			}
		}
		
	    $addressEntry->loadFromArray($this->getData('address'));
        $addressEntry->addressID = $addressID?$addressID:null;
