$.wbs.widgets = {
		...
		toggleLoading: function($el, force){
			...
			if (!$el.find('.request-loading').length || force == 'show'){
				var $loadingDiv = $('<div class="request-loading"><img src="img/ajax-loader32.gif"/><span>Загрузка&hellip;</span></div>');
		...
		requests: function(el){
			...
			/*treeEl.wbsTree({
				json : [
					{ title : 'Все открытые', url: '#/requests/'},
					{ title : 'Мои запросы', url: '#/requests/my/'},
					{ id: 'unreadTitle', title : 'Непрочитан', url: '#/requests/unread/'},
					{ id: 'verificationTitle', title : 'Ожидают подтверждения', url: '#/requests/verification/'},
					{ id: 'trashTitle', title : 'Корзина', url: '#/requests/trash/'},
				]
			});*/
			var $div = $("<div/>");
			var $ul = $("<ul class='ui-tree ui-widget-content'></ul>")
			.append($("<li class='ui-tree-child'><a href='#/requests/all/'>Все открытые</a></li>")
					.append($("<ul/>")
							.append("<li><a href='#/requests/assigned/"+$.wbs.get('current-user-id')+"/' id='myRequestsTitle' "+(parseInt($.wbs.get('unread_count')) > 0?" style='font-weight:bold' ":"")+">Мои запросы "+(parseInt($.wbs.get('unread_count')) > 0?"<span>("+$.wbs.get('unread_count')+")</span>":"")+"</a></li>")
							.append("<li><a href='#/requests/notassigned/'>Исполнитель не назначен</a></li>")
					)		
			)
			...
			$ul.append($("<li class='ui-tree-child'><a href='javascript:void(0)' class='ui-tree-child-non-active'>по статусам:</a></li>").append($ul_child))
			.append("<li class='delimeter'><span></span></li>")
			.append($("<li class='ui-tree-child'><a href='#/requests/archive/'>Архив (закрытые запросы)</a></li>")
					.append($("<ul id='trash-list'/>")
							.append("<li><a href='#/requests/verification/' id='verificationTitle'>Ожидают подтверждения</a></li>")
							.append("<li><a href='#/requests/trash/' id='trashTitle'>Корзина</a></li>")
					)
			)
		...
		/*assigners: function(el){
			...
			.append("<li><a href='#/requests/assigned/"+$.wbs.get('current-user-id')+"/' id='myRequestsTitle'>Мои запросы (<span>"+$.wbs.get('unread_count')+"</span>)</a></li>");
		...
		settings: function(el){
			var $div = $("<div/>"),
			$ul = $("<ul class='ui-tree ui-widget-content'></ul>")
			.append("<li><a href='#/sources/'>Email ящики</a></li>")
			.append("<li><a href='#/forms/'>Формы</a></li>")
			//.append("<li><a href='#/classes/'>Классификаторы</a></li>");
			$div.append($ul);
			el.wbsPortlet({
				title: 'Настройки',
				buttons: ['collapse'],
		...
		menuBtn: function(el){
			...
			$btn.append("<span class='ui-button-text'>Папки▼</span>");
		...
		searchRequest: function(el){
			var defaultText = "Поиск по номеру запроса или имени клиента";
			...
			var $advanced = $('<a href="javascript:void(0)" id="adv-search-link">Расширенный поиск</a>'),
			$adv_search = $('<div id="adv-search" />');
			$adv_search.append("<h2>Расширенный поиск</h2>");
			...
			$usersList.prepend('<option value="-1" selected="selected">&lt;любой&gt;</option>');
			...
			$adv_search.append($("<div class='adv-search-div'/>")
				.append($("<div class='adv-search-fields-div'/>")
						.append("<p class='with-field'><label for='adv-search-name'>Имя клиента:</label><input id='adv-search-name' /></p>")
						.append("<p class='with-field'><label for='adv-search-email'>Email клиента:</label><input id='adv-search-email' /></p>")
						.append("<p class='with-field'><label for='adv-search-id'>Номер клиента:</label><input id='adv-search-id' /></p>")
				)
				.append($("<div class='adv-search-words-div'/>")
					.append("<p class='with-field'><label for='adv-search-words'>Слова:</label><input id='adv-search-words' /></p>")
					.append("<p class='with-check'><input type='checkbox' value='0' checked='checked' class='adv-search-words-mode' disabled='disabled' id='adv-search-in-subject' /><label for='adv-search-in-subject'>в теме</label></p>")
					.append("<p class='with-check'><input type='checkbox' value='1' checked='checked' class='adv-search-words-mode w' disabled='disabled' id='adv-search-in-request' /><label for='adv-search-in-request'>в тексте запроса</label></p>")
					.append("<p class='with-check'><input type='checkbox' value='2' class='adv-search-words-mode w' disabled='disabled' id='adv-search-in-log' /><label for='adv-search-in-log'>в ответах и обсуждениях</label></p>")
				)
				.append($("<div class='adv-search-users-div'/>")
					.append($("<p class='with-field' />")
							.append("<label for='adv-search-users' id='user-list-label'>Пользователь:</label>")
							.append($usersList)
							)
					.append("<p class='with-check'><input type='checkbox' value='0' class='adv-search-users-mode' disabled='disabled' id='adv-users-reply' /><label for='adv-users-reply'>ответил на запрос</label></p>")
					.append("<p class='with-check'><input type='checkbox' value='1' class='adv-search-users-mode' disabled='disabled' id='adv-users-comments' /><label for='adv-users-comments'>выполнял другие действия</label></p>")
				)
			)
			...
			$adv_search_btn_search = $("<button id='adv-search-btn-search'>Искать</button>").button().click(function(){
			...
			$adv_search_btn_cancel = $("<button>Отмена</button>").button().click(function(){
		...
		checkEmail: function($el){
			...
			var $linkText = $("<a href='javascript:void(0)'>"+"Обновить"+"</a>"),
			$linkLoading = $("<img src='img/ajax-loader-w.gif'/><span>&nbsp;Загрузка&hellip;</span>")
		...
		/*buttons1: function(el){
			var $req = $("<button id='new-request'><b>Добавить новый запрос</b></button>");
			$req.button().click(function(){
	    		var hash = '#/requests/add';
	    		hash = hash.replace(/^.*#/, '');
	    		$.historyLoad(hash);
			});
			var $btn = $("<button id='refresh-grid'><b>Обновить</b></button>");
		...
		title: function(el){
			var $content =  $('<h1></h1>'+
					...
					'<span id="requests-count-label"> запросов</span>)</div>');
					/*+
                ...
                for (var key in log) {
                    if (key != 'actions') {
                        ...
                        '<div class="log-message ticket-log-container">' +
	                        ...
	                        //(l.to ? '<p class="action-to"><span class="to-label">Кому:</span><b>'+l.to+'</b></p>' : '')+
	                    '</div>';
                    }
		...
		classChanger: function(id){
			...
			$.wbs.ajaxRequest({
				...
				callback: function(data){
					...
					$btnSave = $('<button>Сохранить</button>').button();
					$btnCancel = $('<button>Отмена</button>').button();
