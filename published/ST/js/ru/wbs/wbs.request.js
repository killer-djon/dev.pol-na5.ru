$.wbs.request = {
	...
	requestSidebar: function(){
        $('.ticket-sidebar').wbsPopup({
		    ...
		    at: "right bottom",
			open:function(){
        		$('.show-right-column').html('скрыть подробности'); 
        	},
			close:function(){
        		$('.show-right-column').html('показать подробно'); 
        	}
        })
	...
	topBtnEvents: function(){
        ...
        if (!$('#gridPanel').is(':visible')){
            $('.maximize').html('Свернуть');
            $('.maximize').button({text: false, icons: {
                primary: 'ui-icon-arrowreturnthick-1-s'
            }});
            $('.backlink').show();
        } else {
            $('.maximize').html('Развернуть');
            $('.maximize').button({text: false, icons: {
		...
		if (!$.browser.msie){
			...
			if ($('.ticket-log-container').length > 0){
				var $selectSort = $('<select></select>')
					.append('<option value="ASC">старые записи наверху</option>')
					.append('<option value="DESC">новые записи наверху</option>'),
				$sortPane = 
					$('<div class="log-order-div"/>')
					.append('<span>Сортировка истории запроса:</span>')
					.append($selectSort);
	...
	toggleQuotes: function(){
	    var show_quote = $('<div class="show-quote" href="javascript:void(0)">- Показать цитируемый текст -</div>').click(function () {
	        if ($(this).hasClass('open')) {
	            $(this).next().hide();
	            $(this).html('- Показать цитируемый текст -').removeClass('open');
	        } else {
	            $(this).next().show();
	            $(this).html('- Скрыть цитируемый текст -').addClass('open');
	        }
	    });
	...
	header: function(){
		...
		$(".request-action").click(function () {
			...
			if (!to) {
				to = '<input type="text" class="autocomplete-'+autocompleteClass+' required" name="to" autocomplete="off" /><button class="contact-search-btn">Выбор контакта</button>';
			}
		    ...
		    switch (type) {
				...
				case 'FORWARD':
		            ...
		            '<tr><td width="1%" class="inp-label">Кому: </td><td>' + 
		            to +    
		            '</td></tr><tr class="copy" style="display: none">'+
		            '<td width="1%" class="inp-label" style="margin-right:10px">Копия: </td>' + 
		            '<td><input type="text" id="copy-input" class="copy-input autocomplete-'+autocompleteClass+'" name="cc" autocomplete="off" /><button class="contact-search-btn">Выбор контакта</button></td>' +
		            '</tr><tr class="bcc" style="display: none">' + 
		            '<td width="1%" class="inp-label">&nbsp;Скрытая копия: </td>' +
		            '<td><input type="text" id="bcc-input" class="bcc-input autocomplete-'+autocompleteClass+'" name="bcc" autocomplete="off" /><button class="contact-search-btn">Выбор контакта</button></td>' + 
		            '</tr><tr><td></td>'+
		            '<td><a href="javascript:void(0)" onclick="$.wbs.request.showChild(this, \'copy\'); $(\'#copy-input\')[0].focus();">Копия</a>'+
		            '<a href="javascript:void(0)" style="margin-left:10px" onclick="$.wbs.request.showChild(this, \'bcc\'); $(\'#bcc-input\')[0].focus(); ">Скрытая копия</a>' + 
		            '</td></tr><tr><td></td>' + 
		            ...
		            //    '<button class="knowledge" style="float:right">Найти ответ в Базе знаний</button>' +
						'</div>'+  
		            '</td></tr>' + 
		            '</table>';
		            break; 
		        case 'FORWARD-ASSIGN':
		            content = '<table width="100%" cellpadding="0" cellspacing="0" border="0" class="form-with-editor">' + 
		            '<tr><td width="1%" class="inp-label"><span class="assign">Кого</span>: </td><td>' + 
		            users +    
		            '</td></tr><tr class="copy" style="display: none">'+
		            '<td width="1%" class="inp-label" style="margin-right:10px">Копия: </td>' + 
		            '<td><input type="text" id="copy-input" class="copy-input autocomplete-'+autocompleteClass+'" name="cc" autocomplete="off" /><button class="contact-search-btn">Выбор контакта</button></td>' +
		        ...
		        case 'ASSIGN': 
		            ...
		            $label = $('<label for="sendAssignMsg">Отправить email-уведомление</label>')
		    ...
		    if (content) {
				...
				var 
		            ...
		            $attach_link = $('<a href="javascript:void(0)" id="attach_link" class="attach_link">Приложить файл</a>'),
		            ...
		            $input.change(function(){
		                ...
		                $deleteBtn = $("<button>Удалить файл</button>").button({text: false, icons: {
				...
				form.submit(function () {
		            ...
		            if (error) {
		                $(this).find('div.error').html('Пожалуйста, заполните обязательные поля').show();
		                return false;
		            } else {
		                ...
		                switch (response.data.type) {
		                    ...
		                    case 'DELETE': 
		                        if ($('.ui-editor-editablefield').length > 0){
		                           $('.ui-editor-editablefield:first').html('<p class="request-deleted-msg" style="text-align:center">Запрос был удален.</p>');
		                        } else {
		                            $('#ticket').html('Запрос был удален.');
		                        }
