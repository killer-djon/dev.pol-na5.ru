...
$.widget("ui.wbsEditor", 	{
	options: {
		...
		buttons: [
		          ...
		          /*[['indent'], ['outdent']],*/
		          [['link', 'Добавить ссылку'], ['unlink', 'Удалить ссылку'], ['hr', 'Горизонтальная линия'] ]
		          ,
		          [['removeFormat', 'Удалить форматирование']]/*,['showHTML']]*/
		],
		...
		switch(command){
				...
				for (key in items){
					fonts.push({
						...
						callback: function(){
							...
							self.doc.execCommand('fontsize', false, parseInt($.wbs.in_array(font, items))+1,10)
							self.textarea.val(self.editablefield.html());
						}
					})
				}
				...
				button.data('onLoad', function($this) {
					...
					var $linkDiv = $("<div/>"), 
						...
						$btn = $("<button>OK</button>").click(function(){
							...
							$linkDiv.wbsPopup('close');
							self.textarea.val(self.editablefield.html());
							return false;
						});
					$linkDiv.append('Введите адрес ссылки:').append($input).append($btn);
					$linkDiv.wbsPopup({
						padding:3,
						width: 200,
				        parent: $this,
			...
			case 'showHTML':
				button.data('clickEvent', function(button) {
					...
					if (visible != 'none'){
						$('button',self.toolbar).button("disable");
	...
	_create: function() {
		...
		this.element.empty()
		this.element.addClass("wbs-resizable ui-editor ui-widget ui-widget-content ui-helper-clearfix ui-corner-all");
		this.toolbar  = $('<div/>')
	...
	showLinkEditDiv: function($link){
		...
		$linkDiv.append('Редактировать адрес ссылки').append($input).append($btn);
		$linkDiv.wbsPopup({
			padding:3,
			width: 300,
	        parent: self.toolbar,
	...
	loadContent: function(params){
		...
		self.setContent('<div class="request-loading"><img src="img/ajax-loader32.gif"/><span>Загрузка&hellip;</span></div>');
		if (params.ajax){
			$.wbs.currentUrl = params.url;
			if (!$.wbs.get('load-uid')){
				$.wbs.set('load-uid',params.uid);
				$.get(params.url, function(data) {
					if ($.wbs.get('load-uid') == params.uid){
						self.element.show();
