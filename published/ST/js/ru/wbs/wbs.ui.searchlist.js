...
$.widget("ui.wbsSearchlist", {
	...
	createItems: function($this, count){
		...
		if (o.filter != 'input' && !self.content.find('option').length && !self.content.find('.empty-results').length)
			self.content.append('<span class="empty-results">Загрузка...</span>')
		if (self.content.find('option').length){
			self.select.append('<span class="empty-results">Загрузка...</span>')
		}
		$.wbs.ajaxRequest({
			url: o.source,
			data: {
			...
			callback: function(data){
				...
				if (!self.content.find('option').length) {
					self.select.remove();
					self.content.append('<span class="empty-results">нет контактов</span>')
				}
	...
	_create: function() {
		...
		if (o.filter == 'input'){
			var $searchBtn = $("<button>Искать</button>").button(),
				$input = $("<input class='ui-autocomplete-input' style='width:315px' />");
			$input.val('Поиск по имени или email');
			...
			this.input.append("<option value='0' selected='selected'>Все папки</option>");
			if (o.filterSource) {
				if (!self.content.find('.empty-results').length)
					self.content.append('<span class="empty-results">Загрузка...</span>')
				$.wbs.ajaxRequest({
		...
		$selectButton = $("<button class='select-btn'>Выбрать</button>");
		this.selectButton = $selectButton;
		this.btnPane.append($selectButton).append('<span class="use-ctrl-msg">Используйте Ctrl+click или Shift+click для выбора нескольких контактов</span>');
		...
		this.element.prepend($('<a href="#" style="position:absolute; top:5px; right: 5px;">Закрыть</a>').click(function(){
			$form.wbsPopup('close');
		}))
