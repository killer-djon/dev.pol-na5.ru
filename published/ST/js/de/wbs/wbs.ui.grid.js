...
var abstractView = {
	...
	createFooter: function(el){
		 $footer = $("<div class='ui-grid-footer ui-widget-header' style='position:relative;'>"+
			"<span id='requests-count-label'>Requests:</span><span id='requests-count'></span>"+
			"<span id='assigned-filter-label'>Show:</span>"+
			"<div id='assigned-filter'>"+
			"<a id='assigned-open' href='#'>open</a> | <a id='assigned-all' href='#'>all</a>"+
			"</div>"+
			/*"<div id='pages-label'><span>Pages:</span> <span id='pager'></span></div>"+
			"<div id='grid-size-label' style='font-size:11px;margin-top:1px'><span>Show</span>" +
			"<select id='grid-size'>" +
...
gridData = {
	...
	appendRows: function(offset, callback){
		...
		var $loading = ("<tr class='loading-row'><td colspan='10'><div style='width:100%; min-height:20px; text-align: center;'><img src='img/ajax-loader-w.gif'/>Loading&hellip;</div></td></tr>")
		...
		$.wbs.ajaxRequest({
	    	...
	    	callback: function(data){
				...
				if (data.requests.length > 0) {
					...
					$('tbody', self.grid).append("<tr class='ui-grid-no-rows'><td colspan='10'><div>-- end of the list --</div></td></tr>")
				}
				self.cloneHeader();
			}
		})
	...
	loadPage: function(callback){
		...
		//}
		self.grid.html('<div style="height:'+$('#mainPage').height()+'px; margin-left:20px" class="grid-loading">Loading&hellip;</div>');
	...
	tdOnLoad: function(el){
		...
		if ($this.hasClass('ui-grid-column-new_window')){
			$this.attr('title','Open in new window');
		}
