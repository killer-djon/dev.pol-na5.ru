...
(function($) {
	$.wbs.controller = {
		...
		requestsSearchAction: function(attr){
			...
			$('#title h1').html('[`Search results for `] "'+attr[0]+'"');
    	    document.title = '[`Search results`] — [`Support Tracker`]';
		},
		...
		requestsAdvsearchAction: function(attr){
        	...
        	for (var key in attr){
        		...
        		if (key>0 && attr[key-1] == "wmode"){
        			...
        			var subject = '';
        			if (has_subject) subject = '[`subject`] [`and`] ';
        			switch (attr[key]){
        				case '1':
                			search_str += '[`in`] '+ subject +'[`request text`]';
                			$('#adv-search-in-request').attr('checked','checked');
                			break;
        				case '2':
                			search_str += '[`in`] '+ subject +'[`replies and discussion`]';
                			$('#adv-search-in-log').attr('checked','checked');
                			break;
        				case '3':
        					if (has_subject) {
        						search_str += '[`in`] [`subject`], [`request text`], [`replies and discussion`]';
        					} else {
        						search_str += '[`in`] [`request text`], [`replies and discussion`]';
        					}
                			$('#adv-search-in-log').attr('checked','checked');
                			$('#adv-search-in-request').attr('checked','checked');
                			break;
        		...
        		if (key>0 && attr[key-1] == "umode"){
        			...
        			switch (attr[key]){
        				case '1':
                			search_str += '[`replied to request`]';
                			$('#adv-users-reply').attr('checked','checked');
                			break;
        				case '2':
                			search_str += '[`performed other actions`]';
                			$('#adv-users-comments').attr('checked','checked');
                			break;
        				case '3':
                			search_str += '[`replied to request`] [`or`] [`performed other actions`]';
                			$('#adv-users-comments').attr('checked','checked');
                			$('#adv-users-reply').attr('checked','checked');
                			break;
        			}
		...
		showEmptyBinDialog: function(state){
    		$('.refresh-link').hide();
			if (state == '-1') {
				var defaultText = '[`Empty trash`]';
			} else {
				var defaultText = '[`Empty this folder`]';
			}
			var $emptyBinLink = $('<a class="empty-bin-link" href="javascript:void(0)">'+defaultText+'</a>').css('float','right'),
			$dialog = $(
				'<p>[`All requests will be removed and you will not be able to restore them.`]<br/><br/>'+
				'[`Remove requests?`]</p>'
			);
			$emptyBinLink.click(function(){
				if (!$('.grid-tbody tr').length){
					$("<p>[`Folder is empty`]</p>").dialog({
						modal:true,
						title: '[`Confirmation`]',
	                    buttons: {
	                        OK: function() {
		...
		formsAddAction: function () {
			$.post('?m=forms&act=save', {'add':1, 'name': '[`New form`]'}, function (response) {
