...
b.pageX),Math.abs(this._mouseDownEvent.pageY-b.pageY))>=this.options.distance},_mouseDelayMet:function(){return this.mouseDelayMet},_mouseStart:function(){},_mouseDrag:function(){},_mouseStop:function(){},_mouseCapture:function(){return true}})})(jQuery);(function(a){var b,c=function(f){a(":ui-button",f.target.form).each(function(){var e=a(this).data("button");setTimeout(function(){e.refresh()},1)})},d=function(f){var e=f.name,g=f.form,h=a([]);if(e)h=g?a(g).find("[name='"+e+"']"):a("[name='"+e+"']",f.ownerDocument).filter(function(){return!this.form});return h};a.widget("ui.button",{options:{text:true,label:null,icons:{primary:null,secondary:null}},_create:function(){this.element.closest("form").unbind("reset.button").bind("reset.button",c);this._determineButtonType();
...
k/2;if(e.at[1]==="bottom")m.top+=j;else if(e.at[1]==="center")m.top+=j/2;m.left+=i[0];m.top+=i[1];return this.each(function(){var l=a(this),p=l.outerWidth(),n=l.outerHeight(),o=a.extend({},m);if(e.my[0]==="right")o.left-=p;else if(e.my[0]==="center")o.left-=p/2;if(e.my[1]==="bottom")o.top-=n;else if(e.my[1]==="center")o.top-=n/2;a.each(["left","top"],function(r,q){a.ui.position[h[r]]&&a.ui.position[h[r]][q](o,{targetWidth:k,targetHeight:j,elemWidth:p,elemHeight:n,offset:i,my:e.my,at:e.at})});a.fn.bgiframe&&
l.bgiframe();l.offset(a.extend(o,{using:e.using}))})};a.ui.position={fit:{left:function(e,g){var h=a(window);h=e.left+g.elemWidth-h.width()-h.scrollLeft();e.left=h>0?e.left-h:Math.max(0,e.left)},top:function(e,g){var h=a(window);h=e.top+g.elemHeight-h.height()-h.scrollTop();e.top=h>0?e.top-h:Math.max(0,e.top)}},flip:{left:function(e,g){if(g.at[0]!=="center"){var h=a(window);h=e.left+g.elemWidth-h.width()-h.scrollLeft();var i=g.my[0]==="left"?-g.elemWidth:g.my[0]==="right"?g.elemWidth:0,k=-2*g.offset[0];
...
c?a(window).width()+"px":b+"px"}else return a(document).width()+"px"},resize:function(){var b=a([]);a.each(a.ui.dialog.overlay.instances,function(){b=b.add(this)});b.css({width:0,height:0}).css({width:a.ui.dialog.overlay.width(),height:a.ui.dialog.overlay.height()})}});a.extend(a.ui.dialog.overlay.prototype,{destroy:function(){a.ui.dialog.overlay.destroy(this.$el)}})})(jQuery);(function(a){function b(d){return typeof d=="object"?d:{top:d,left:d}}var c=a.scrollTo=function(d,f,e){a(window).scrollTo(d,f,e)};c.defaults={axis:"xy",duration:parseFloat(a.fn.jquery)>=1.3?0:1};c.window=function(){return a(window)._scrollable()};a.fn._scrollable=function(){return this.map(function(){if(!(!this.nodeName||a.inArray(this.nodeName.toLowerCase(),["iframe","#document","html","body"])!=-1))return this;var d=(this.contentWindow||this).document||this.ownerDocument||this;return a.browser.safari||
d.compatMode=="BackCompat"?d.body:d.documentElement})};a.fn.scrollTo=function(d,f,e){if(typeof f=="object"){e=f;f=0}if(typeof e=="function")e={onAfter:e};if(d=="max")d=9E9;e=a.extend({},c.defaults,e);f=f||e.speed||e.duration;e.queue=e.queue&&e.axis.length>1;if(e.queue)f/=2;e.offset=b(e.offset);e.over=b(e.over);return this._scrollable().each(function(){function g(p){i.animate(m,f,e.easing,p&&function(){p.call(this,d,e)})}var h=this,i=a(h),k=d,j,m={},l=i.is("html,body");switch(typeof k){case "number":case "string":if(/^([+-]=)?\d+(\.\d+)?(px|%)?$/.test(k)){k=
b(k);break}k=a(k,this);case "object":if(k.is||k.style)j=(k=a(k)).offset()}a.each(e.axis.split(""),function(p,n){var o=n=="x"?"Left":"Top",r=o.toLowerCase(),q="scroll"+o,s=h[q],t=c.max(h,n);if(j){m[q]=j[r]+(l?0:s-i.offset()[r]);if(e.margin){m[q]-=parseInt(k.css("margin"+o))||0;m[q]-=parseInt(k.css("border"+o+"Width"))||0}m[q]+=e.offset[r]||0;if(e.over[r])m[q]+=k[n=="x"?"width":"height"]()*e.over[r]}else{o=k[r];m[q]=o.slice&&o.slice(-1)=="%"?parseFloat(o)/100*t:o}if(/^\d+$/.test(m[q]))m[q]=m[q]<=0?
0:Math.min(m[q],t);if(!p&&e.queue){s!=m[q]&&g(e.onAfterFirst);delete m[q]}});g(e.onAfter)}).end()};c.max=function(d,f){var e=f=="x"?"Width":"Height",g="scroll"+e;if(!a(d).is("html,body"))return d[g]-a(d)[e.toLowerCase()]();e="client"+e;var h=d.ownerDocument.documentElement,i=d.ownerDocument.body;return Math.max(h[g],i[g])-Math.min(h[e],i[e])}})(jQuery);/*
...
setHash:function(b){b=a.History.format(b);b=b.replace(/^\/?|\/?(\?)|\/?$/g,"/$1");if(typeof window.location.hash==="undefined")location.hash=b;if(a.browser.msie&&parseInt(a.browser.version,10)<8){a.History.$iframe.contentWindow.document.open();a.History.$iframe.contentWindow.document.close()}},go:function(b){var c=a.History;b=c.format(b);if(c.getHash()!==b)c.setHash(b);else{c.setState(b);c.trigger()}return true},hashchange:function(){var b=a.History;b.options.debug&&console.debug("History.hashchange",
this,arguments);var c=b.getHash(),d=b.getState();if(!b.$iframe&&d===c||b.$iframe&&b.hash===b.$iframe.contentWindow.document.location.hash)return false;if(d===c)return false;b.setState(c);b.trigger();return true},bind:function(b,c){var d=a.History;if(c){if(typeof d.handlers.specific[b]==="undefined")d.handlers.specific[b]=[];d.handlers.specific[b].push(c)}else{c=b;d.handlers.generic.push(c)}return true},trigger:function(b){var c=a.History;if(typeof b==="undefined")b=c.getState();var d,f,e,g;if(typeof c.handlers.specific[b]!==
"undefined"){g=c.handlers.specific[b];d=0;for(f=g.length;d<f;++d){e=g[d];e(b)}}g=c.handlers.generic;d=0;for(f=g.length;d<f;++d){e=g[d];e(b)}return true},construct:function(){var b=a.History;a(document).ready(function(){b.domReady()});return true},configure:function(b){var c=a.History;c.options=a.extend(c.options,b);return true},domReadied:false,domReady:function(){var b=a.History;if(!b.domRedied){b.domRedied=true;b.$window=a(window);b.$window.bind("hashchange",this.hashchange);setTimeout(b.hashchangeLoader,
200);return true}},hashchangeLoader:function(){var b=a.History;if(a.browser.msie&&parseInt(a.browser.version)>=8)(c=b.getHash())&&b.$window.trigger("hashchange");else{if(a.browser.msie){b.$iframe=a('<iframe id="jquery-history-iframe" style="display: none;"></$iframe>').prependTo(document.body)[0];b.$iframe.contentWindow.document.open();b.$iframe.contentWindow.document.close();var c=b.getHash();if(c)b.$iframe.contentWindow.document.location.hash=c;c=function(){var d=b.format(b.$iframe.contentWindow.document.location.hash);
b.getState()!==d&&b.setHash(b.$iframe.contentWindow.document.location.hash);d=b.getHash();b.getState()!==d&&b.go(d)}}else c=function(){var d=b.getHash();b.getState()!==d&&b.go(d)};a.browser.msie&&parseInt(a.browser.version)<8?setInterval(c,1500):setInterval(c,200)}return true}};a.History.construct()})(jQuery);jQuery.fn.autocomplete=function(a,b){return this.each(function(){function c(n){window.clearInterval(g);if(n!=e&&b.minChars!=null&&n.length>=b.minChars){var o={};e=o[b.parameters.name]=n;$.wbs.ajaxRequest({url:a,data:o,callback:l});j=false}}var d=$(this),f=$('<ul class="autocomplete"></ul>');$(d).wrap("<div class='autocomplete-wrapper' style='position:relative'/>");$(d).after(f);f.css({position:"absolute","max-height":"400px","overflow-y":"auto"});var e="",g,h=0,i=0,k="",j=true;b=jQuery.extend({minChars:3,
limit:15,timeout:300,parameters:{name:"text"},selection:"<b>$1</b>",hiddenField:false,insert:function(n,o){n.val(o)},value:function(n){return n.val()}},b);var m,l=function(n){m=n;i=-1;var o="";$(".autoload-loading").remove();if(n){(h=n.length)||f.hide();for(var r in n)if(typeof n[r]=="string"){var q=$("<div></div>").html(n[r]).html(),s=b.value(d);if(s.replace(/\s/g,"").length>0)q=q.replace(RegExp("("+s.replace(/^\s\s*/,"").replace(/\s\s*$/,"").replace(/\s/g,"|")+")","gi"),b.selection);o+='<li value="'+
r+'">'+q+"</li>";f.html(o);f.show().css({top:d.outerHeight(),left:0,width:d.outerWidth()-2});f.children().hover(function(){$(this).addClass("selected").siblings().removeClass("selected")},function(){$(this).removeClass("selected")}).click(function(){b.insert(d,$(this).text());b.hiddenField&&$("#"+b.hiddenField).val($(this).attr("value"));p()});f.children(":first").addClass("selected");i=0;$.browser.msie&&$("select:visible").hide().addClass("autoHide")}}};$(document).click(function(n){if(f.is(":visible"))$(n.target).isChildrenOf(f)||
$(n.target).isChildrenOf(d.parent())||p()});var p=function(){f.find("*").remove();f.hide();h=0;i=-1};d.keydown(function(n){if(n.which==13){if(f.is(":hidden"))c(b.value(d));else{b.insert(d,f.children().eq(i).text());b.hiddenField&&$("#"+b.hiddenField).val(f.children().eq(i).attr("value"));p()}n.preventDefault();return false}else n.which==9&&p();new_value=b.value(d);if(new_value.indexOf(k)==-1)j=true;k=new_value});d.keyup(function(n){if(n.which!=16){window.clearTimeout(g);if(n.which==27||n.which==9)p();
else if(n.which==13)return false;else if(n.which==40||n.which==38){switch(n.which){case 40:i=i>=h-1?0:i+1;break;case 38:i=i<=0?h-1:i-1}f.children().removeClass("selected").eq(i).addClass("selected").text()}else{b.value(d).length==0&&p();j||(j=b.value(d).length>b.minChars);if(j||n.which==46||n.which==8||f.children().length==0||f.children().length>=b.limit)g=window.setTimeout(function(){c(b.value(d))},b.timeout);if(m==undefined)return false;for(var o in m)if(typeof m[o]=="string"){n=b.value(d).toLowerCase().split(" ");
for(var r=false,q=0;q<n.length;q++)r=r||m[o].toLowerCase().indexOf(n[q])==-1;if(r){delete m[o];o--}}f.children().length<b.limit&&l(m)}b.value(d).length||p()}})})};$.widget("ui.wbsSplitter",$.ui.mouse,{options:{initSize:100,max:1E3,min:-1E3,zIndex:1E3,orientation:"vertical",step:1,value:0,collapsed:false,collapsedValue:0,handleSize:7,handleBorder:2},_create:function(){var a=this.options;this._detectOrientation();this._mouseInit();this.element.addClass("ui-widget").addClass("ui-widget-content").addClass("ui-splitter ui-splitter-"+this.orientation);a.disabled&&this.element.addClass("ui-splitter-disabled ui-disabled");a.value=a.initSize;this.element.css({position:"relative"});
a.orientation=="vertical"?this.element.css({cursor:"e-resize","z-index":a.zIndex}):this.element.css({cursor:"n-resize","z-index":2E3});this.dragged=false;this.element.click(function(b){b.preventDefault()}).dblclick(function(b){b.preventDefault()}).hover(function(){a.disabled||$(this).addClass("ui-state-hover")},function(){$(this).removeClass("ui-state-hover")}).focus(function(){if(a.disabled)$(this).blur();else{$(".ui-splitter.ui-state-focus").removeClass("ui-state-focus");$(this).addClass("ui-state-focus")}}).blur(function(){$(this).removeClass("ui-state-focus")})},
destroy:function(){this.handles.remove();this.range.remove();this.element.removeClass("ui-splitter ui-splitter-horizontal ui-splitter-vertical ui-splitter-disabled ui-widget ui-widget-content ui-corner-all").removeData("slider").unbind(".slider");this._mouseDestroy();return this},_mouseCapture:function(a){if($(a.target).is("#"+$(this.element).attr("id"))){this.startOffset=this.element.offset();this.nextElSize={width:this.element.next().width(),height:this.element.next().height()};this.prevElSize=
{width:this.element.prev().width(),height:this.element.prev().height()};this.dragged=!this.dragged;this.elementSize={width:this.element.outerWidth(),height:this.element.outerHeight()};this.elementOffset=this.element.offset();var b={x:a.pageX,y:a.pageY};this._start(a);this.element.addClass("ui-state-active").focus();this._clickOffset={left:b.x-this.startOffset.left,top:b.y-this.startOffset.top};normValue=this._normValueFromMouse(b);this._split(a,normValue);return true}else return false},_mouseStart:function(){return true},
_mouseDrag:function(a){var b=this._normValueFromMouse({x:a.pageX,y:a.pageY});this._split(a,b);return false},_mouseStop:function(a){dragged=false;this.element.removeClass("ui-state-focus");this.element.removeClass("ui-state-active");this._stop(a);this._change(a);this._clickOffset=null;return false},_detectOrientation:function(){this.orientation=this.options.orientation=="vertical"?"vertical":"horizontal"},_normValueFromMouse:function(a){return"vertical"==this.orientation?a.x-this.elementOffset.left-
(this._clickOffset?this._clickOffset.left:0):a.y-this.elementOffset.top-(this._clickOffset?this._clickOffset.top:0)},_start:function(a){this._trigger("_start",a,this.value())},_split:function(a,b){b!=this.value()&&this._trigger("split",a,b)!==false&&this.value(b)},_stop:function(a){$prev=this.element.prev();$prev.id=$prev.attr("id");var b=$.wbs.getClosestLayout($prev.id),c=$.wbs.getObjByKey($prev.id,b.layout,true);if(this.options.orientation=="vertical"){this.element.css({left:0});c.width=(this.prevElSize.width+
this.options.value).toString()}else{this.element.css({top:0});c.height=(this.prevElSize.height+this.options.value).toString()}b.resizeAll();typeof $.cookie=="function"&&this.element.attr("id")=="splitter2"&&$.cookie("grid-height",$("#grid .ui-grid-wrapper:first").height()+30);this._trigger("_stop",a,{value:this.value(),id:this.element.attr("id")})},_change:function(a){this._trigger("_change",a,this.value())},value:function(a){if(arguments.length){this.options.value=this._trimValue(a);this.refresh();
this._change(null,0)}return this._value()},_setOption:function(a,b){$.Widget.prototype._setOption.apply(this,arguments);switch(a){case "disabled":if(b){this.handles.filter(".ui-state-focus").blur();this.handles.removeClass("ui-state-hover");this.handles.attr("disabled","disabled");this.element.addClass("ui-disabled")}else{this.handles.removeAttr("disabled");this.element.removeClass("ui-disabled")}case "orientation":this._detectOrientation();this.element.removeClass("ui-splitter-horizontal ui-splitter-vertical").addClass("ui-splitter-"+
this.orientation);this.refresh();break;case "value":this.refresh()}},_step:function(){return this.options.step},_value:function(){var a=this.options.value;return a=this._trimValue(a)},_trimValue:function(a){var b=this.options;if(a<this._valueMin())a=this._valueMin();if(a>this._valueMax())a=this._valueMax();if(this.orientation=="horizontal"&&a>$(this.element).parent().width()-b.handleSize)a=$(this.element).parent().width()-b.handleSize;if(this.orientation=="vertical"&&a>$(this.element).parent().height()-
b.handleSize)a=$(this.element).parent().height()-b.handleSize;return a},_valueMin:function(){return this.options.min},_valueMax:function(){return this.options.max},refresh:function(a){var b=this.options;if(typeof a=="undefined")a=this.value();b.orientation=="vertical"?this.element.css({left:a}):this.element.css({top:a})}});$.widget("ui.wbsPortlet",{options:{initSize:200,buttons:["collapse"],title:"<b>HTML</b>",content:false,collapsed:false,disabled:false},_swapIcons:function(a){a[2]=a[0];a[0]=a[1];a[1]=a[2];return a},_create:function(){var a=this.options;this.element.empty().addClass("wbs-resizable ui-portlet ui-widget ui-widget-content ui-helper-clearfix");a.disabled&&this.element.addClass("ui-disabled");this.header=$("<div/>",{"class":"ui-portlet-header"}).append(a.title).addClass("ui-widget-header");for(buttonKey in a.buttons){var b;
switch(a.buttons[buttonKey]){case "collapse":b=$('<span class="ui-icon"></span>').addClass("ui-icon-minus").click(function(){$(this).toggleClass("ui-icon-minus").toggleClass("ui-icon-plus");$(this).parents(".ui-portlet:first").find(".ui-portlet-content").toggle()});a.collapsed&&b.click();break;case "action":b=$('<span class="ui-icon"></span>').addClass("ui-icon-triangle-1-s").click(function(){alert("action")})}this.header.prepend(b)}this.content=$("<div/>",{"class":"ui-portlet-content ui-widget-body"});
a.content&&this.content.append($(a.content));this.element.append(this.header).append(this.content)}});$.fn.extend({isChildrenOf:function(a){var b=this,c=a.find("*"),d=false;if($(b).not(a).length)c.each(function(){$(b).not(this).length||(d=true)});else d=true;return d}});
...
"#"+b).attr("rel",b);b.hover(function(){$(this).addClass("ui-wbs-shadow-light")},function(){$(this).removeClass("ui-wbs-shadow-light")});b.mousedown(a.callback);this.wrapper.append(b)}this.wrapper.append("<div style='clear:both'></div>");this.wrapper.width(a.elementsInRow*18);this.element.append(this.wrapper)}});$.widget("ui.wbsEditor",{options:{showToolbar:true,buttons:[[["bold","[`Bold`]"],["italic","[`Italic`]"],["underline","[`Underline`]"],["strikethrough","[`Strikeout`]"]],[["forecolor","[`Font Color`]"],["backcolor","[`Background Color`]"]],[["left","[`Left Justify`]"],["center","[`Center`]"],["right","[`Right Justify`]"]],[["ol","[`Numbered List`]"],["ul","[`Bullets`]"]],[["link","[`Insert Link`]"],["unlink","[`Remove Link`]"],["hr","[`Horizontal Rule`]"]],[["removeFormat","[`Remove Format`]"]]],
useSimpleButtons:false,simpleBtns:[[["bold","[`Bold`]"],["italic","[`Italic`]"],["underline","[`Underline`]"]],[["forecolor","[`Font color`]"],["backcolor","[`Background color`]"]],[["left"],["center"],["right"]],[["ol"],["ul"]],[["indent"],["outdent"]],[["link"],["unlink"]]],fill:true,attachFiles:false,editable:true,withFrame:false,menuOffset:false,height:300,name:""},addButton:function(a,b,c,d){var f=this,e=this.options,g=$("<button/>"),h={x:0,y:0};if(e.menuOffset)h={x:0,y:25};switch(b){case "fontname":var i=
...
m.wbsPopup({padding:3,parent:j,absolute:true})});break;case "link":g.data("onLoad",function(j){j.wrap('<div style="position: relative;float:left;"></div>');var m=$("<div/>"),l=$("<input value='http://' class='disable-selection-on-focus' style='width:150px' />"),p=$("<button>OK</button>").click(function(){f.doc.execCommand("createlink",false,l.val());m.wbsPopup("close");f.textarea.val(f.editablefield.html());return false});m.append("[`Enter link address:`]").append(l).append(p);m.wbsPopup({padding:3,
...
""))});break;case "img":g.data("onLoad",function(j){j.wrap('<div style="position: relative;float:left;"></div>');var m=$("<div/>"),l=$("<input type='file' name='files[]' />").clone();l.change(function(){var p=l.clone(),n=$("<button>X</button>").button().click(function(){o.remove();m.wbsPopup("open")}),o=$("<div/>").append(p).append(n);m.prepend(o);l.val("")});m.append(l);m.wbsPopup({padding:3,width:200,parent:j,absolute:true,offset:h})});g.data("clickEvent",function(){return false});b="insertimage";
...
'"]').text():$('label[for="'+c.attr("id")+'"]').text();f=f.replace(":","");c.keyup(function(){d=c.is(":input")?c.val().trim():c.text().trim();switch(a){case "required":var e="err_"+c.attr("id");if(d==""){if(!$("#"+e).length){$(".errors-msgbox",c.closest("form")).append('<p id="'+e+'">[`Field`] "'+f+'" [`is required`]</p>');c.addClass("error ui-state-error")}}else{$("#err_"+c.attr("id")).remove();c.removeClass("error ui-state-error")}break;case "any":var g=true,h="";for(key in b){h=b[key];h=h.is(":input")?
h.val().trim():h.text().trim();if(h!="")g=false}e=h="";var i=[];h=$("<p>[`One of this fields should not be empty`]: </p>");i.push('"'+f+'"');for(key in b){e=b[key].hasClass("ui-editor-editablefield")?$('label[for="'+b[key].parent().attr("id")+'"]').text():$('label[for="'+b[key].attr("id")+'"]').text();e=e.replace(":","");i.push('"'+e+'"')}e="err_"+$.wbs.implode("",i.sort()).replace(/"/gi,"");if(d==""&&g){if(!$("#"+e).length){h.append($.wbs.implode(", ",i.sort()));h.attr("id",e);$(".errors-msgbox",
...
-1){a(this).addClass("ui-state-highlight");var h=a(this).text();a("#title h1").html(h);document.title=h+" \u2014 [`Helpdesk`]"}});var f="",e=b.length;for(hashId in b){var g=b[hashId];if(hashId<2)if(hashId==0)f=g;else f+=g.substr(0,1).toUpperCase()+g.substr(1);else{e=hashId;break}}for(c in gridData.data.headers.data){g=gridData.data.headers.data[c];if(g.options.cssClass.search("ui-grid-th-assigned_c_id")!=-1||g.options.cssClass.search("ui-grid-th-state_id")!=-1||g.options.cssClass.search("ui-grid-th-new_window")!=
...
b[0]=a.wbs.trim(b[0]);if(decodeURIComponent){b[0] = decodeURIComponent(b[0]);}gridData.requestData.search=b[0];a.wbs.pages.mainPage("search="+b[0]);a("#searchRequestInput").val(b[0]);a("#title h1").html('[`Search results for `] "'+b[0]+'"');document.title="[`Search results`] \u2014 [`Support Tracker`]"},requestsAdvsearchAction:function(b){a.cookie("single-panel-view")=="true"&&a.cookie("single-panel-view","false");var c="",d="",f=false,e;for(e in b){b[e]=a.wbs.trim(b[e],false);if(e>0&&b[e-1]=="name"){c+="|name|"+b[e];d+="[`and`] [`client name`] <b>"+b[e]+
"</b> ";a("#adv-search-name").val(b[e])}if(e>0&&b[e-1]=="email"){c+="|email|"+b[e];d+="[`and`] [`client email`] <b>"+b[e]+"</b> ";a("#adv-search-email").val(b[e])}if(e>0&&b[e-1]=="clientid"){c+="|clientid|"+b[e];d+="[`and`] [`client Id`] <b>"+b[e]+"</b> ";a("#adv-search-id").val(b[e])}if(e>0&&b[e-1]=="subject"){c+="|subject|"+b[e];f=true;d+="[`and`] [`words`] <b>"+b[e]+"</b> ";a.wbs.in_array("words",b)||(d+="[`in`] [`subject`]");a("#adv-search-in-subject").attr("checked","checked");a("#adv-search-in-request").removeAttr("checked");
a(".adv-search-words-mode").removeAttr("disabled");a("#adv-search-words").val(b[e])}if(e>0&&b[e-1]=="words"){c+="|words|"+b[e];f||(d+="[`and`] [`words`] <b>"+b[e]+"</b> ");a("#adv-search-words").val(b[e]);a(".adv-search-words-mode").removeAttr("disabled")}if(e>0&&b[e-1]=="wmode"){c+="|wmode|"+b[e];var g="";if(f)g="[`subject`] [`and`] ";switch(b[e]){case "1":d+="[`in`] "+g+"[`request text`]";a("#adv-search-in-request").attr("checked","checked");break;case "2":d+="[`in`] "+g+"[`replies and discussion`]";
a("#adv-search-in-log").attr("checked","checked");break;case "3":d+=f?"[`in`] [`subject`], [`request text`], [`replies and discussion`]":"[`in`] [`request text`], [`replies and discussion`]";a("#adv-search-in-log").attr("checked","checked");a("#adv-search-in-request").attr("checked","checked")}d+=" "}if(e>0&&b[e-1]=="user"){c+="|user|"+b[e];g="";var h=a.wbs.get("users"),i;for(i in h)if(h[i].id==b[e])g=h[i].name;d+="[`and`] [`user`] <b>"+g+"</b> ";a("#adv-search-users").val(b[e])}if(e>0&&b[e-1]=="umode"){c+=
"|umode|"+b[e];switch(b[e]){case "1":d+="[`replied to request`]";a("#adv-users-reply").attr("checked","checked");break;case "2":d+="[`performed other actions`]";a("#adv-users-comments").attr("checked","checked");break;case "3":d+="[`replied to request`] [`or`] [`performed other actions`]";a("#adv-users-comments").attr("checked","checked");a("#adv-users-reply").attr("checked","checked")}d+=" "}}delete gridData.requestData.filter;c=c.substr(1,c.length);d=d.replace("[`and`]","");gridData.requestData.search=
c;a.wbs.pages.mainPage(c);a("#title h1").html("[`Search results`]");$search_params=a('<div id="search-params"><span>'+d+"</span></div>");a("label, span",$search_params).click(function(){a("#adv-search").wbsPopup("open")});a("#search-params").length||a("#title").append($search_params);a("#adv-search-btn-search").button("enable");document.title="[`Search results`] \u2014 [`Support Tracker`]"},requestsAllAction:function(){a.wbs.pages.mainPage("filter=open")},requestsStateAction:function(b){a.wbs.pages.mainPage("filter=state-"+
b[0])},showEmptyBinDialog:function(b){a(".refresh-link").hide();var c=a('<a class="empty-bin-link" href="javascript:void(0)">'+(b=="-1"?"[`Empty trash`]":"[`Empty this folder`]")+"</a>").css("float","right"),d=a("<p>[`All requests will be removed and you will not be able to restore them.`]<br/><br/>[`Remove requests?`]</p>");c.click(function(){a(".grid-tbody tr").length?d.dialog({modal:true,title:"[`Confirmation`]",buttons:{"[`Yes`]":function(){a.wbs.ajaxRequest({url:"?m=requests&act=emptybin",requestMethod:"post",
data:{state:b},callback:function(){gridData.updatePage();a("#textarea > div").empty();d.dialog("close")}})},"[`No`]":function(){a(this).dialog("close")}},zIndex:1E4}):a("<p>[`Folder is empty`]</p>").dialog({modal:true,title:"[`Confirmation`]",buttons:{OK:function(){a(this).dialog("close")}},zIndex:1E4})});c.css({top:a("#mainPage").offset().top+10});a("body").append(c)},requestsTrashAction:function(){a.wbs.pages.mainPage("filter=trash");this.showEmptyBinDialog(-1)},requestsVerificationAction:function(){a.wbs.pages.mainPage("filter=verification");
...
formsAddAction:function(){a.post("?m=forms&act=save",{add:1,name:"[`New form`]"},function(b){b.status=="OK"&&a.wbs.loadHash("/forms/info/"+b.data.id)},"json");return false},formsInfoAction:function(b){a("#forms-controls").show();var c=function(){a(".form-preview").show();a(".form-edit").hide()};a("#form-tabs").length?c():a.wbs.pages.loadPage("?m=forms&act=info&id="+b[0],c)},formsEditAction:function(b){var c=function(){a("#forms-controls").hide();var d=b[1]||"fields";a("#form-tabs li").removeClass("tabs-selected");
...
this;$("#client_requests_link").click(function(){$.wbs.loadHash("#requests/advsearch/clientid/"+a.data.client_c_id+"/id/"+a.data.id)});$("#client_name").click(function(){$(".contact-panel").toggle()})},requestSidebar:function(){$(".ticket-sidebar").wbsPopup({padding:3,parentShadow:false,parent:$(".show-right-column"),absolute:true,appendToBody:true,my:"right top",at:"right bottom",open:function(){$(".show-right-column").html("[`hide details`]")},close:function(){$(".show-right-column").html("[`show details`]")}});
$(".ticket-sidebar").click(function(){$(this).wbsPopup("close")});$("td:first",$("tr",$(".right-panel-block table"))).css({"text-align":"right","padding-right":"5px"})},topBtnEvents:function(){$("#detail").click(function(){$("#detail").text("[`Loading...`]");$.wbs.widgets.toggleTabPanel();return 0});$(".backlink").click(function(){$("#close:first").click()});if($("#gridPanel").is(":visible")){$(".maximize").html("[`Expand`]");$(".maximize").button({text:false,icons:{primary:"ui-icon-extlink"}});$(".backlink").hide()}else{$(".maximize").html("[`Minimize`]");
$(".maximize").button({text:false,icons:{primary:"ui-icon-arrowreturnthick-1-s"}});$(".backlink").show()}maximize_click=function(){$(".backlink").toggle();if($("#gridPanel").is(":visible")){$(".maximize").button("option","label","[`Minimize`]");$(".maximize span").addClass("ui-icon-arrowreturnthick-1-s").removeClass("ui-icon-extlink")}else{$(".maximize").button("option","label","[`Expand`]");$(".maximize span").addClass("ui-icon-extlink").removeClass("ui-icon-arrowreturnthick-1-s")}$.wbs.widgets.toggleTopPanel()&&
...
$("#toolbarbuttons",toolbarClone).hide();if($("#container").length)$("#ticket").after(toolbarClone);else{$("body").append(toolbarClone);$(".backlink").remove()}if($(".ticket-log-container").length>0){var d=$("<select></select>").append('<option value="ASC">[`oldest entries first`]</option>').append('<option value="DESC">[`newest entries first`]</option>');b=$('<div class="log-order-div"/>').append("<span>[`Sort request history`]:</span>").append(d);toolbarClone.append(b);$("option[value="+a.data.order+
...
autocompleteClass="contacts";var g="",h={};if(f.hasOwnProperty("properties"))if(f.properties.length>0){if((h=$.parseJSON(f.properties))&&h.hasOwnProperty("autocomplete"))autocompleteClass=h.autocomplete;if(h&&h.hasOwnProperty("to")&&h.to=="users")g=users}g||(g='<input type="text" class="autocomplete-'+autocompleteClass+' required" name="to" autocomplete="off" /><button class="contact-search-btn">[`Select contact`]</button>');var i="";switch(e){case "REPLY":g='<div class="reply-to-msg">'+a.data.client_from+
'<input name="to" autocomplete="off" value="'+a.data.client_from+'" type="hidden"/></div>';case "FORWARD":i='<table width="100%" cellpadding="0" cellspacing="0" border="0" class="form-with-editor"><tr><td width="1%" class="inp-label">[`To`]: </td><td>'+g+'</td></tr><tr class="copy" style="display: none"><td width="1%" class="inp-label" style="margin-right:10px">[`Copy`]: </td><td><input type="text" id="copy-input" class="copy-input autocomplete-'+autocompleteClass+'" name="cc" autocomplete="off" /><button class="contact-search-btn">[`Select contact`]</button></td></tr><tr class="bcc" style="display: none"><td width="1%" class="inp-label">&nbsp;[`Bcc`]: </td><td><input type="text" id="bcc-input" class="bcc-input autocomplete-'+
autocompleteClass+'" name="bcc" autocomplete="off" /><button class="contact-search-btn">[`Select contact`]</button></td></tr><tr><td></td><td><a href="javascript:void(0)" onclick="$.wbs.request.showChild(this, \'copy\'); $(\'#copy-input\')[0].focus();">[`Copy`]</a><a href="javascript:void(0)" style="margin-left:10px" onclick="$.wbs.request.showChild(this, \'bcc\'); $(\'#bcc-input\')[0].focus(); ">[`Bcc`]</a></td></tr><tr><td></td><td><div class="wbs-editor"></div></td></tr><tr><td></td><td><div class="error" style="display:none"></div><div class="button-pane"><button class="save">[`Send`]</button><button class="cancel">[`Cancel`]</button></div></td></tr></table>';
break;case "FORWARD-ASSIGN":i='<table width="100%" cellpadding="0" cellspacing="0" border="0" class="form-with-editor"><tr><td width="1%" class="inp-label">[`<span class="assign">To</span>`]: </td><td>'+users+'</td></tr><tr class="copy" style="display: none"><td width="1%" class="inp-label" style="margin-right:10px">[`Copy`]: </td><td><input type="text" id="copy-input" class="copy-input autocomplete-'+autocompleteClass+'" name="cc" autocomplete="off" /><button class="contact-search-btn">[`Select contact`]</button></td></tr><tr><td></td><td><a href="javascript:void(0)" onclick="$.wbs.request.showChild(this, \'copy\'); $(\'#copy-input\')[0].focus();">[`Copy`]</a></td></tr><tr><td></td><td>';
if(f.hasOwnProperty("properties"))if(f.properties.length>0)if((h=$.parseJSON(f.properties))&&h.hasOwnProperty("comment"))i+='<div style="margin-bottom: 10px">'+h.comment+"</div>";i+='<textarea class="comment-text" name="text" rows="5" style="width: 98%"></textarea></td></tr><tr><td></td><td><div class="error" style="display:none"></div><div class="button-pane"><button class="save">[`Send`]</button><button class="cancel">[`Cancel`]</button></div></td></tr></table>';break;case "ASSIGN":$to=$(users).css("width",
"100%");$editor=$("<textarea rows='5' style='width:420px;padding:10px;' name='text'></textarea>");$to.change(function(){$("option[value=0]",$to).remove();$to.val()==0?$(".ui-dialog .ui-button:first").button("disable"):$(".ui-dialog .ui-button:first").button("enable")});$label=$('<label for="sendAssignMsg">[`Send notification`]</label>');i=$("<div style='padding-left:10px'/>").append($to).append($("<div/>").append($editor));break;case "COMMENT":i="";if(h.copy)i='<table width="100%" cellpadding="0" cellspacing="0" border="0" class="form-with-editor"><tr class="copy"><td width="1%" class="inp-label" style="margin-right:10px">[`Copy`]: </td><td><input type="text" id="copy-input" class="copy-input autocomplete-'+
autocompleteClass+'" name="cc" autocomplete="off" /><button class="contact-search-btn">[`Select contact`]</button></td></tr></table>';i+=h.editor&&h.editor=="html"?'<div class="wbs-editor comment-text"></div>':'<textarea class="comment-text" name="text" rows="5" style="width: 98%"></textarea>';i+='<div class="button-pane"><button class="save">[`Send`]</button><button class="cancel">[`Cancel`]</button></div>'}if(i){$(".ticket-toolbar-bottom").hide();f='<div class="action-header header" style="border-bottom-color:'+
...
$(".assignDialog").clone();j.empty();j.append(f);j.dialog({title:"[`Change assigment`]",width:500,modal:true,buttons:{"[`Assign`]":function(){$(".ajax-form .ui-editor").wbsEditor("setTextareaContent");k.submit();j.remove()},"[`Cancel`]":function(){j.remove()}},zIndex:1E4});$("textarea",f).focus();a.data.assigned_c_id==0&&$(".ui-dialog .ui-button:first").button("disable");j.css("padding",0);$(".action-header",j).hide();j.css("min-height",$(".action-form",j).height())}$(".save").click(function(){$(".wbs-editor").wbsEditor("setTextareaContent");
...
0?$(".ui-editor-editablefield:first").html('<p class="request-deleted-msg" style="text-align:center">[`Request has been deleted.`]</p>'):$("#ticket").html("[`Request has been deleted.`]");typeof gridData!=="undefined"&&gridData.deleteBlock(a.data.id);break;default:$(".ui-editor-editablefield").length>0?$(".ui-editor-editablefield").load("?m=requests&act=info&id="+a.data.id):location.reload()}else $("<div>"+l.error+"</div>").dialog({modal:true,buttons:{OK:function(){$(this).dialog("close")}},zIndex:1E5});
...
$("<div/>"),c=$("<ul class='ui-tree ui-widget-content'></ul>").append($("<li class='ui-tree-child'><a href='#/requests/all/'>[`All open`]</a></li>").append($("<ul/>").append("<li><a href='#/requests/assigned/"+$.wbs.get("current-user-id")+"/' id='myRequestsTitle' "+(parseInt($.wbs.get("unread_count"))>0?" style='font-weight:bold' ":"")+">[`My requests`] "+(parseInt($.wbs.get("unread_count"))>0?"<span>("+$.wbs.get("unread_count")+")</span>":"")+"</a></li>").append("<li><a href='#/requests/notassigned/'>[`Not assigned`]</a></li>"))).append("<li class='delimeter'><span></span></li>"),
d=$.wbs.get("data").states,f=$("<ul class='ui-tree-child'/>"),e;for(e in d)d[e].id>0&&d[e].group==-101&&f.append("<li><a href='#/requests/state/"+d[e].id+"/'>"+d[e].name+"</a></li>");c.append($("<li class='ui-tree-child'><a href='javascript:void(0)' class='ui-tree-child-non-active'>[`by status`]:</a></li>").append(f)).append("<li class='delimeter'><span></span></li>").append($("<li class='ui-tree-child'><a href='#/requests/archive/'>[`Archive (closed requests)`]</a></li>").append($("<ul id='trash-list'/>").append("<li><a href='#/requests/verification/' id='verificationTitle'>[`Waiting verification`]</a></li>").append("<li><a href='#/requests/trash/' id='trashTitle'>[`Trash`]</a></li>")));
b.append(c);a.wbsPortlet({title:"[`Requests`]",buttons:["collapse"],content:b});$("#settingsPanel").length||$("#trash-list").remove()},settings:function(a){var b=$("<div/>"),c=$("<ul class='ui-tree ui-widget-content'></ul>").append("<li><a href='#/sources/'>[`Email boxes`]</a></li>").append("<li><a href='#/forms/'>[`Forms`]</a></li>");b.append(c);a.wbsPortlet({title:"[`Settings`]",buttons:["collapse"],content:b})},toggleLeftPanel:function(){if($("#leftPanel").is(":visible")){$("#leftPanel").hide();
...
"30px");$.cookie("left-panel","visible")}$("#container").layout("resizeAll")},menuBtn:function(a){var b=$('<div id="main-menu-div"/>').css({position:"absolute","margin-top":"6px"});a.append(b);var c=$("<button class='main-menu-btn'/>").button();$(".ui-button-text",c).remove();c.append("<span class='ui-button-text'>[`Folders`]\u25bc</span>");$menu=$("<div id='main-menu'/>");$items=$("#leftMenu").children().clone(true);b.append(c);$(".ui-portlet-header span",$items).remove();var d=$("<div id='main-menu-wrapper'/>");
...
10)},35)},function(){clearInterval(f)});e.hover(function(){f=setInterval(function(){$menu.scrollTop($menu.scrollTop()-10)},50)},function(){clearInterval(f)})}},close:function(){clearInterval(f);$("#main-menu-up-arr, #main-menu-down-arr").remove()}});d.prepend($("#toggle-main-menu",$menu));$("a",$items).click(function(){d.wbsPopup("close")});a.hide()},searchRequest:function(a){var b="[`Search by request number or client1s name`]";b=b.replace("1","'");var c=$('<input style="width:300px; padding:4px;margin-right:5px;"value="'+
b+'" type="text" id="searchRequestInput" />'),d=$("<button>[`Search`]</button>");d.button({icons:{primary:"ui-icon-search"}});c.click(function(){$(this).val()==b&&$(this).val("")}).blur(function(){$(this).val().split(" ").join("")==""&&$(this).val(b)}).keypress(function(j){j.which==13&&d.click()});d.click(function(){gridData.requestData.offset=0;if(c.val()!=b&&c.val().split(" ").join("")!=""){var j="#/requests/search/"+$.wbs.trim(c.val())+"/";$.wbs.loadHash(j)}else{delete gridData.requestData.search;
c.val(b)}});var f=$('<a href="javascript:void(0)" id="adv-search-link">[`Advanced search`]</a>'),e=$('<div id="adv-search" />');e.append("<h2>[`Advanced search`]</h2>");var g=$("<select id='adv-search-users'></select>");for(key in $.wbs.get("users"))g.append("<option value="+$.wbs.get("users")[key].id+">"+$.wbs.get("users")[key].name+"</option>");Array.prototype.sort.call($("option",g),function(j,m){return $(j).text().toLowerCase()>$(m).text().toLowerCase()?1:-1}).appendTo(g);g.prepend('<option value="-1" selected="selected">[`&lt;anyone&gt;`]</option>');
g.change(function(){if($(this).val()=="-1"){$(".adv-search-users-mode").attr("disabled","disabled");$(".adv-search-users-mode").removeAttr("checked")}else $(".adv-search-users-mode").removeAttr("disabled")});e.append($("<div class='adv-search-div'/>").append($("<div class='adv-search-fields-div'/>").append("<p class='with-field'><label for='adv-search-name'>[`Client name`]:</label><input id='adv-search-name' /></p>").append("<p class='with-field'><label for='adv-search-email'>[`Client email`]:</label><input id='adv-search-email' /></p>").append("<p class='with-field'><label for='adv-search-id'>[`Client Id`]:</label><input id='adv-search-id' /></p>")).append($("<div class='adv-search-words-div'/>").append("<p class='with-field'><label for='adv-search-words'>[`Words`]:</label><input id='adv-search-words' /></p>").append("<p class='with-check'><input type='checkbox' value='0' checked='checked' class='adv-search-words-mode' disabled='disabled' id='adv-search-in-subject' /><label for='adv-search-in-subject'>[`in subject`]</label></p>").append("<p class='with-check'><input type='checkbox' value='1' checked='checked' class='adv-search-words-mode w' disabled='disabled' id='adv-search-in-request' /><label for='adv-search-in-request'>[`in request text`]</label></p>").append("<p class='with-check'><input type='checkbox' value='2' class='adv-search-words-mode w' disabled='disabled' id='adv-search-in-log' /><label for='adv-search-in-log'>[`in replies and discussions`]</label></p>")).append($("<div class='adv-search-users-div'/>").append($("<p class='with-field' />").append("<label for='adv-search-users' id='user-list-label'>[`User`]:</label>").append(g)).append("<p class='with-check'><input type='checkbox' value='0' class='adv-search-users-mode' disabled='disabled' id='adv-users-reply' /><label for='adv-users-reply'>[`replied to request`]</label></p>").append("<p class='with-check'><input type='checkbox' value='1' class='adv-search-users-mode' disabled='disabled' id='adv-users-comments' /><label for='adv-users-comments'>[`performed other actions`]</label></p>")));
...
"/clientid/"+$.wbs.trim($("#adv-search-id").val()):"")+(l!=""&&$("#adv-search-in-subject").is(":checked")?"/subject/"+$.wbs.trim($("#adv-search-words").val(),true):"")+(j?j:"")+(m?m:"");return $.wbs.trim(j)},i=$("<button id='adv-search-btn-search'>[`Search`]</button>").button().click(function(){var j=h();if(j!=""){$.wbs.loadHash("/requests/advsearch/"+j);e.wbsPopup("close")}});i.button("disable");e.find("input").keyup(function(j){if(h()=="")i.button("disable");else{i.button("enable");j.which==13&&
i.click()}}).bind("paste",function(){$(this);setTimeout(function(){h()==""?i.button("disable"):i.button("enable")},100)});$adv_search_btn_cancel=$("<button>[`Cancel`]</button>").button().click(function(){var j=k;e.wbsPopup("close");$("#adv-search-name").val($("#adv-search-name",j).val());$("#adv-search-email").val($("#adv-search-email",j).val());$("#adv-search-id").val($("#adv-search-id",j).val());$("#adv-search-words").val($("#adv-search-words",j).val());$("#adv-search-in-subject",j).is(":checked")?
...
""?i.button("disable"):i.button("enable")})},checkEmail:function(a){var b=$("<a href='javascript:void(0)'>[`Refresh`]</a>"),c=$("<img src='img/ajax-loader-w.gif'/><span>&nbsp;[`Loading`]&hellip;</span>");$link=$("<div class='refresh-link'/>");$link.append(b);a.append($link);$link.click(function(){if($link.find("a").length){$link.empty().append(c);$.get("?m=requests&act=check&force=1",function(d){d=$.parseJSON(d);$link.empty().append(b);if(d.status=="ERR")for(key in d.error)$.wbs.widgets.showAlertMsg($("#topPanel"),
d.error[key]);gridData.updatePage()})}});return a},button:function(a,b){a.button(b);return a},transform:function(a,b){var c=a.attr("class");a=$("<"+b.tag+"/>");for(var d in b)typeof a[d]=="function"&&a[d](b[d]);a.attr("class",c);return a},title:function(a){var b=$('<h1></h1><div class="requests-count-div" style="display:none">(<span id="requests-count">0</span><span id="requests-count-label"> [`requests`]</span>)</div>');a.css("width","100%");a.append(b)},grid:function(a){a.wbsGrid()},toggleTabPanel:function(){if($("#tabs-toolbar").length){if(typeof $defaultContent!=
"undefined"){var a=$defaultContent.clone(true);$("#ticket").replaceWith(a)}$("#detail").text("[`show details`]");$(".buttons").prepend($("#toolbarbuttons"));$("#tabs-toolbar").remove();$("#info-container").show();$(".ticket-toolbar").show()}else{$(".ticket-toolbar").before('<div id="tabs-toolbar"></div>');$.ajax({url:"?m=requests&act=tabs",data:{id:$.wbs.get("current-request-id")},dataType:"html",success:function(b){$("#tabs-toolbar").css({position:"relative",display:"block",height:"32px","z-index":"100"});
$("#detail").text("[`hide details`]");$("#tabs-toolbar").append(b);$("#tabs-toolbar").append($("#toolbarbuttons"));$("#textarea").wbsEditor("resize")}})}$("#textarea").wbsEditor("resize")},toggleBottomPanel:function(a,b){if($("#bottomPanel").is(":visible")||a=="hide"){$("#gridPanel").is(":visible")||this.toggleTopPanel();$bottomPanel=$("#bottomPanel");$gridSplitter=$("#splitter2");$("#bottomPanel").hide();$("#splitter2").hide();gridPanelHeight=$("#gridPanel").height();$("#gridPanel").height($("#mainPage").height()-
$("#toolbar").outerHeight());$("#grid").wbsGrid("resize");if(typeof scrollTop!="undefined"&&$(".ui-grid-wrapper").length)$(".ui-grid-wrapper")[0].scrollTop=scrollTop;$("#grid").addClass("tr-cursor-pointer");b||$.cookie("single-panel-view","true");return"[`Close`]"}if(!$("#bottomPanel").is(":visible")||a=="show"){$("#bottomPanel").show();$("#splitter2").show();$("#gridPanel").height(gridPanelHeight);$("#grid").wbsGrid("resize");b||$.cookie("single-panel-view","false");return"[`Maximize`]"}},toggleTopPanel:function(a){if($("#gridPanel").is(":visible")||
...
'"><span class="date"> '+b.datetime+'</span><span title="" class="author"> '+b.contact+' </span><span class="action"> '+b.log_name+" </span>"+(b.to?'<span class="action">'+b.to+"</span>":"")+"</div>"+(b.text?'<div class="text ui-corner-bottom">'+b.text+"</div>":"")+"</div>")},showChangesInRequests:function(){(new Date).getTime();$btn=$('<a href="#">[`Reload page`]</a>');$closeBtn=$('<div style="float:right; cursor:pointer" class="close-log-message-btn ui-icon ui-icon-closethick"></div>');var a=0,
...
else{e.append($btn);e.prepend($closeBtn);$("#info-container").append(e);$closeBtn.click(function(){e.remove()})}}$btn.click(function(){$btn.html("[`Loading`]...");$("#grid").length?$("#grid").wbsGrid("selectBlock",$.wbs.get("current-request-id")):location.reload()});if($.wbs.controller.currentAction!="requestsAdvsearch"&&$.wbs.controller.currentAction!="requestsSearch"){if(c.updated!=""){if(typeof gridData!="undefined"){f="";for(d in c.updated)if($(".ui-grid-column-id[rel='"+c.updated[d].data.id.data+
...
g).val($(this).val())});$dialog.dialog({modal:true,title:"[`Change classifiers`]",zIndex:1E4,open:function(g){$parent=$(g.target).parent();var h=$parent.width();g=Math.round($(window).width()/2);h=(g-h)/2;$parent.width(g);$parent.offset({left:$parent.offset().left-h})}})}})}};$.wbs.pages={abstractPage:function(a,b,c,d){$panel=$("#"+b);$panel.layout("destroy");$panel.empty();$panel.layout({layoutMarkupId:c,onResize:d,callback:function(){$("#title h1").html($(".ui-tree .ui-state-highlight:first").text());$("#leftPanel").show();$.cookie("left-panel")=="hidden"&&$.wbs.widgets.toggleLeftPanel()}})},mainPage:function(a){$.wbs.getLayout("container");delete gridData.requestData.lastreqlogid;delete gridData.requestData.lastreqid;delete gridData.requestData.lastlogid;delete gridData.requestData.lastid;
delete gridData.requestData.limit;delete gridData.requestData.offset;var b=/\/id\//.exec(parent?parent.window.location.hash:location.hash);if($("#mainPage",$("#contentPanel")).length==0){$.wbs.set("grid-filter",a);this.abstractPage("container","contentPanel","mainPage",function(){$("#grid").wbsGrid("resize");$("#textarea").wbsEditor("resize")});if($.cookie("single-panel-view")=="true"){$.wbs.widgets.toggleBottomPanel();if(b&&$("#topPanel").is(":visible")){$.wbs.widgets.toggleBottomPanel();$.wbs.widgets.toggleTopPanel();
$.wbs.set("full-view",true)}}}else{var c=$("#grid"),d=$.wbs.get("grid-filter");$.wbs.set("grid-filter",a);$(".act-items").remove();$("#textarea").wbsEditor("setContent","");if(a!=d){if($("#bottomPanel").is(":visible")&&!$("#gridPanel").is(":visible"))$.wbs.widgets.toggleBottomPanel();else $("#gridPanel").is(":visible")||$.wbs.widgets.toggleTopPanel();b||$.wbs.set("current-request-id",0);$.wbs.set("last_req_log_id","99999999");$("#search-params").remove();c.wbsGrid("setGridData");$("#textarea").wbsEditor("empty")}else{if($("#bottomPanel").is(":visible"))$.cookie("single-panel-view")==
"true"&&$("#bottomPanel").is(":visible")&&$.wbs.widgets.toggleBottomPanel();else if(b){$.wbs.widgets.toggleBottomPanel();$.wbs.widgets.toggleTopPanel()}c.wbsGrid("selectBlock",$.wbs.get("current-request-id"));$.wbs.set("last_req_log_id","99999999")}}},loadPage:function(a,b){$("#pageContainer",$("#contentPanel")).length==0&&this.abstractPage("container","contentPanel","pageContainer",function(){$("#grid").wbsGrid("resize");$("#textarea").wbsEditor("resize")});$("#checkEmail .refresh-link").hide();
$("#pageContainer").load(a,function(){$(":button, :submit, input[type=button]").button();a=="?m=classes"&&$.wbs.widgets.classesContainer();$.wbs.widgets.toggleLoading("body","hide");typeof b=="function"&&b()})}};dragged=false;
$.widget("ui.wbsSearchlist",{options:{filter:"input",source:"?m=contacts&act=list",filterSource:false,loadOnCreate:false,disableScroll:false,hideInput:false,onSelect:function(){return false}},createItems:function(a,b){var c=this,d=this.options,f,e;f=b?b+", 30":"30";if($("input",a).length){e=$("input:first",a).val();$("input:first",a).addClass("ui-autocomplete-loading")}else{e=a.val();a.addClass("ui-autocomplete-loading")}d.filter!="input"&&!c.content.find("option").length&&!c.content.find(".empty-results").length&&c.content.append('<span class="empty-results">[`Loading`]...</span>');
c.content.find("option").length&&c.select.append('<span class="empty-results">[`Loading`]...</span>');$.wbs.ajaxRequest({url:d.source,data:{text:e,limit:f},callback:function(g){c.content.find(".empty-results").remove();a.removeClass("ui-autocomplete-loading");$("input:first",a).removeClass("ui-autocomplete-loading");if(!c.content.find("select").length){var h=$("<select multiple='multiple' style='width:100%;height:100px'/>");c.content.append(h);c.select=h;if(!d.disableScroll){c.select.unbind();c.select.scroll(function(){var k=
$(this);if(!c.appending_rows)if(this.scrollHeight-100-this.scrollTop<10){c.appending_rows=true;c.createItems(c.input,k.find("option").length)}})}}for(key in g){var i=g[key];h=$("<option/>");h.append(i);h.addClass("ui-state-default");h.dblclick(function(){c.selectButton.click()});h.click(function(){c.selectButton.button("enable")});i=c.select.scrollTop();c.select.append(h);c.select.scrollTop(i)}if(!c.content.find("option").length){c.select.remove();c.content.append('<span class="empty-results">[`no contacts`]</span>')}c.appending_rows=
false}});return true},_create:function(){var a=this,b=this.options;this.content=$("<div class='box ui-widget-content'/>");this.btnPane=$("<div class='ui-widget-content'/>");if(b.filter=="input"){var c=$("<button>[`Search`]</button>").button(),d=$("<input class='ui-autocomplete-input' style='width:315px' />");d.val("[`Search by name or email`]");d.focus(function(){this.value=""});d.keypress(function(e){e.which==13&&c.click()});this.input=$("<div />").append(d).append(c);c.click(function(){a.content.empty();
a.createItems(d)});if(b.loadOnCreate){d.val("");a.content.find(".empty-results").length||a.content.append('<span class="empty-results">[`Loading`]...</span>');a.createItems(d)}b.hideInput&&this.input.hide()}else{this.input=$("<select class='input-p100 ui-autocomplete-input'></select>");this.input.append("<option value='0' selected='selected'>[`All folders`]</option>");if(b.filterSource){a.content.find(".empty-results").length||a.content.append('<span class="empty-results">[`Loading`]...</span>');
$.wbs.ajaxRequest({url:b.filterSource,callback:function(e){a.content.find(".empty-results").remove();a.input.removeClass("ui-autocomplete-loading");for(key in e){var g=e[key],h=$("<option/>"),i="";g=g.split("|");for(var k=0;k<parseInt(g[0]);k++)i+="&nbsp;&nbsp;";i+=g[1];h.append(i);h.val(key);a.input.append(h)}a.createItems(a.input)}})}this.input.change(function(){a.content.empty();a.createItems(a.input)})}this.selectButton=$selectButton=$("<button class='select-btn'>[`Select`]</button>");this.btnPane.append($selectButton).append('<span class="use-ctrl-msg">[`Use Ctrl+click or Shift+click to select multiple contacts`]</span>');
this.element.addClass("ui-widget");$selectButton.button();$.browser.msie||$selectButton.button("disable");var f=$("#contact-search-form.ui-popup");this.element.prepend($('<a href="#" style="position:absolute; top:5px; right: 5px;">[`Close`]</a>').click(function(){f.wbsPopup("close")}));$selectButton.click(function(){var e=[];$("option:selected",a.content).each(function(h){e[h]=$.wbs.trim($(this).text())});e=$.wbs.implode(", ",e);var g=f.wbsPopup("option","selectedParent").prev();if(g.get(0).tagName.toLowerCase()!=
"input")g=$("input",f.wbsPopup("option","selectedParent").prev());g.val()!=""?g.val(g.val()+", "+e):g.val(e);g.val().length>55&&g.css("width",$("#action-container").width()-500);f.wbsPopup("close")});a.appending_rows=false;this.element.append(this.input);this.input.wrap("<div class='input-wrapper'>");this.element.append(this.content);this.element.append(this.btnPane)}});var abstractView={layoutMarkup:"<div id='wrapper'><div id='tableWrapper'><div id='gridBody'><div id='headerWrapper'><div id='headers' class='wrapped'/></div><div id='blocksBody'><div id='blocks' class='multiply wrapped'/></div></div></div></div>",create:function(a){var b=this;this.root=$("<div/>");this.layout=$("<div id='root'>"+this.layoutMarkup+"</div>");this.options=a;$("div",this.layout).each(function(){var c=$(this).parent().attr("id"),d=$(this).attr("id");if($(this).hasClass("multiply")){b[d]=
[];for(var f in a[d].data)typeof b[$.wbs.camelCase(["create",d])]=="function"&&b[$.wbs.camelCase(["create",d])](a[d].data[f])}else if(typeof b[$.wbs.camelCase(["create",d])]=="function")b[d]=b[$.wbs.camelCase(["create",d])](a[d]);b.addParams($(b[d]),a[d]);d=$.wbs.getObjByKey(d,b);c=$.wbs.getObjByKey(c,b);$(this).hasClass("wrapped")?$.wbs.collectSiblings(d).appendToDeepest(c):c.append(d)});return this.root.children()},createHeaders:function(a){var b=[];for(headerId in a.data){var c=a.data[headerId],
d=this.createHeader(c);this.addParams(d,a);this.addParams(d,c);b[b.length]=d}return b},createBlocks:function(a){$blockWrapper=this.createBlockWrapper();this.addParams($blockWrapper,this.options.blockWrapper);if(typeof a!="undefined")for(blockElemId in a.data){var b=this.createBlock(a.data[blockElemId]);this.addParams(b,this.options.blocks);this.addParams(b,a);this.addParams(b,a.data[blockElemId]);b.appendTo($blockWrapper);gridData.tdOnLoad(b);b.wrapInner("<div/>");this.blocks[this.blocks.length]=
$blockWrapper}return $blockWrapper},addParams:function(a,b){if(b&&b.options){b=b.options;if(b.css)typeof b.css=="string"?a.css($.parseJSON(b.css)):a.css(b.css);b.after&&a.append(b.after);b.before&&a.prepend(b.before);b.cssClass&&a.addClass(b.cssClass);if(b.attrs)for(attrKey in b.attrs)a.attr(attrKey,b.attrs[attrKey]);b.linked&&a.addClass("linked-"+b.linked)}},renderBlock:function(a,b){$(".ui-grid-column-id[rel='"+a+"']").parent().replaceWith(this.createBlocks(b));a==$.wbs.get("current-request-id")&&
$(".ui-grid-column-id[rel='"+a+"']").parent().mousedown()},deleteBlock:function(a,b){if($(".ui-grid-column-id").length>1&&b)$(".ui-grid-column-id[rel='"+a+"']").parent().next().length?$("td:first",$(".ui-grid-column-id[rel='"+a+"']").parent().next()).mousedown():$("td:first",$(".ui-grid-column-id[rel='"+a+"']").parent().prev()).mousedown();$(".ui-grid-column-id[rel='"+a+"']").parent().remove();parseInt($("#requests-count").html())-1>=0&&$("#requests-count").html(parseInt($("#requests-count").html())-
1);$("#alertPanel").length&&$.wbs.ajaxRequest({url:"?m=requests&act=count",callback:function(c){c.limit>c.count&&location.reload()}})},addBlock:function(a){var b=this,c=parseInt(a.data.id.data,10),d=0,f=gridData.requestData.order;if($(".grid-tbody tr").length)if(f!="asc"&&c>parseInt($(".grid-tbody .ui-grid-column-id:first").attr("rel"),10)||f=="asc"&&c<parseInt($(".grid-tbody .ui-grid-column-id:first").attr("rel"),10))$(".ui-grid-column-id[rel='"+c+"']").length||$(".grid-tbody").prepend(b.createBlocks(a));
else if((f!="asc"&&c<parseInt($(".grid-tbody .ui-grid-column-id:last").attr("rel"),10)||f=="asc"&&c>parseInt($(".grid-tbody .ui-grid-column-id:last").attr("rel"),10))&&(gridData.requestData.limit>parseInt($("#requests-count").html())||$(".grid-tbody .ui-grid-column-id").length==parseInt($("#requests-count").html())))$(".grid-tbody .ui-grid-column-id:last").parent().after(b.createBlocks(a));else{var e=false;$(".grid-tbody tr").each(function(){if(!e){var g=$(this),h=parseInt($(".ui-grid-column-id",
g).attr("rel"),10);if(f!="asc"&&d>c&&h<c||f=="asc"&&d<c&&h>c){g.before(b.createBlocks(a));e=true}d=h}})}else $(".grid-tbody").append(b.createBlocks(a))},appendBlocks:function(a){this.blocks=[];for(var b in a)this.createBlocks(a[b]);$(".grid-tbody:first").append($.wbs.collectSiblings(this.blocks))},renderBlocks:function(a){$("tbody").find("*").remove();this.blocks=[];for(var b in a)this.createBlocks(a[b]);$(".grid-tbody:first").append($.wbs.collectSiblings(this.blocks))},createTableWrapper:function(a){a=
$("<div class='ui-grid-wrapper' style='overflow-y: scroll; zoom:100%;'></div>");a.scroll(function(){gridData.scrollEvent(this)});return a},createGridBody:function(){return $("<table cellspacing='0' border='0' class='ui-grid-content ui-widget-content'></table>")},createHeaderWrapper:function(){return $("<thead><tr></tr></thead>")},createHeader:function(a){return $("<th nowrap='nowrap' onmousedown='gridData.thMouseDown(this)' class='ui-state-default'>"+a.data+(a.data!="&nbsp;"?"<span></span>":"")+"</th>")},
createBlocksBody:function(){return $("<tbody class='grid-tbody'></tbody>")},createBlockWrapper:function(){return $("<tr onmousedown='gridData.trMouseDown(this)'></tr>")},createBlock:function(a){if(a.data)a.data=a.data.replace(/\&amp\;/g,"&");return $("<td onmousedown='gridData.tdMouseDown(this)'>"+a.data+"</td>")},createWrapper:function(a){return a=$("<div class='ui-widget'></div>")},createFooter:function(){$footer=$("<div class='ui-grid-footer ui-widget-header' style='position:relative;'><span id='requests-count-label'>[`Requests`]:</span><span id='requests-count'></span><span id='assigned-filter-label'>[`Show`]:</span><div id='assigned-filter'><a id='assigned-open' href='#'>[`open`]</a> | <a id='assigned-all' href='#'>[`all`]</a></div></div>");
if(location.hash.search("assigned")>-1)location.hash.search("all")>-1?$("#assigned-all",$footer).addClass("active"):$("#assigned-open",$footer).addClass("active");else{$("#assigned-filter",$footer).remove();$("#assigned-filter-label",$footer).remove()}$("#assigned-open",$footer).click(function(){$.wbs.removeFromHash("all")});$("#assigned-all",$footer).click(function(){$.wbs.addToHash("all")});return $footer}};
gridData={source:"ajax",url:"?m=requests&act=list",limit:20,pageNumber:1,requestData:{},grid:false,appendRows:function(a,b){var c=this;$.wbs.set("appending-rows",true);$("tbody",c.grid).append("<tr class='loading-row'><td colspan='10'><div style='width:100%; min-height:20px; text-align: center;'><img src='img/ajax-loader-w.gif'/>[`Loading`]&hellip;</div></td></tr>");var d=$.wbs.get("grid-filter").split("=");if(typeof d[1]!="undefined")c.requestData[d[0]]=d[1];$.wbs.ajaxRequest({url:c.url,data:$.extend(c.requestData,
{offset:a}),callback:function(f){$(".loading-row").remove();abstractView.appendBlocks(f.requests);typeof b=="function"&&b();f.requests.length>0?$.wbs.set("appending-rows",false):$("tbody",c.grid).append("<tr class='ui-grid-no-rows'><td colspan='10'><div>-- [`end of the list`] --</div></td></tr>");c.cloneHeader()}})},updatePage:function(a){var b=this;$.wbs.set("appending-rows",false);b.requestData.offset=0;var c=Math.ceil(($(window).height()-150)/20);if(c<12)c=12;b.requestData.limit=c;$.wbs.ajaxRequest({url:b.url,
data:b.requestData,callback:function(d){abstractView.renderBlocks(d.requests);typeof a=="function"&&a();b.cloneHeader();b.setFooterData(d)}})},checkForIrrelevantRow:function(a,b,c,d){var f="",e=$.wbs.get("users"),g=$.wbs.get("current-user-id");for(key in $.wbs.get("users"))if(e[key].id==g)f=e[key].name;var h=function(i,k,j){var m=$.wbs.get("data").states,l=parent?parent.window.location.hash:location.hash;if(k=="0"&&l.indexOf("verification")>0||k=="-1"&&l.indexOf("trash")>0)return true;if(i!=$.wbs.get("current-request-id")&&
(k=="-1"||m[k].group!="-101"&&l.indexOf("archive")==-1||m[k].group!="-102"&&l.indexOf("archive")>0||k!=$.wbs.controller.currentActionAttr[0]&&l.indexOf("state")>0||j!=f&&l.indexOf("/assigned")>0||j!=""&&l.indexOf("/notassigned")>0)){abstractView.deleteBlock(i,false);gridData.insertNewBlocks(i,$.wbs.get("last_req_id"))}};a?$(".grid-tbody tr").each(function(){var i=$(this),k,j=$.wbs.get("data").states,m=$(".ui-grid-column-state_id div",i).text(),l;for(l in j)if(j[l].name==m)k=j[l].id;$(".ui-grid-column-id",
i).length&&h($(".ui-grid-column-id",i).attr("rel"),k,$(".ui-grid-column-assigned_c_id div",i).text())}):h(b,c,d)},updateBlock:function(a,b){var c=this;$(".ui-grid-column-id[rel="+a+"]",c.grid).length&&$.wbs.ajaxRequest({url:"?m=requests&act=list&singlerow="+a,data:c.requestData,callback:function(d){abstractView.renderBlock(a,d.requests[0]);typeof b=="function"&&b();c.cloneHeader()}})},updateBlockByData:function(a,b){if($(".ui-grid-column-id[rel="+a.data.id.data+"]",this.grid).length){abstractView.renderBlock(a.data.id.data,
a);this.cloneHeader();typeof b=="function"&&b()}},deleteBlock:function(a,b){abstractView.deleteBlock(a,true);typeof b=="function"&&b()},insertNewBlocks:function(a,b){var c=this;if(a){delete c.requestData.timestamp;var d=$.extend(c.requestData,{refreshed:a})}else{delete c.requestData.refreshed;d=$.extend(c.requestData,{lastid:b})}delete d.offset;delete d.limit;$.wbs.ajaxRequest({url:c.url,data:d,callback:function(f){for(var e in f.requests)abstractView.addBlock(f.requests[e]);c.setFooterData(f);typeof callback==
"function"&&callback();c.requestData.timestamp&&delete c.requestData.timestamp;c.requestData.refreshed&&delete c.requestData.refreshed}})},setFooterData:function(a){gridData.count=Math.ceil(a.count/gridData.limit);$("#requests-count").html(a.count);if(a.count!="0"){$(".first-message:visible").length&&$.wbs.widgets.toggleBottomPanel(false,true);$(".first-message").hide();$("#first-message").remove();$("#table").show()}$.wbs.get("use_limit")==1&&$.wbs.ajaxRequest({url:"?m=requests&act=count",callback:function(b){if(b.limit>
0)if(b.limit>=b.count)$("#alertPanel").length&&location.reload();else $("#alertPanel").length||location.reload()}})},cloneHeader:function(){if(!$.browser.msie){var a=false;if($(".ui-grid-header-clone").length){$(".ui-grid-header-clone").parent().remove();a=true}var b=$(".ui-grid-wrapper table",self.grid).clone(true).removeAttr("id").addClass("ui-grid-header-clone");a&&b.css({position:"relative",top:"0"});$("tbody",b).removeClass("grid-tbody");$(".ui-grid-wrapper",this.grid).before(b);b.wrap("<div style='padding-right:16px; border:none; overflow:hidden; height:"+
$(".ui-table thead tr:first").outerHeight(true)+"px' class='ui-state-default'></div>");a||$(".ui-grid-wrapper table",this.grid).css({position:"relative",overflow:"hidden",top:"-30px"})}},loadPage:function(a){var b=this;if(typeof $.wbs.get("grid-filter")!="undefined"){var c=$.wbs.get("grid-filter").split("=");b.requestData[c[0]]=c[1]}else delete b.requestData.filter;delete abstractView.layout;$.wbs.set("appending-rows",false);$(".requests-count-div").css("display","none");b.grid.html('<div style="height:'+
$("#mainPage").height()+'px; margin-left:20px" class="grid-loading">[`Loading`]&hellip;</div>');val=$.cookie($.cookie("currentAction")+"-sort");if(val!=null){b.requestData.sort=val;b.requestData.order=$.cookie($.cookie("currentAction")+"-order")}else{b.requestData.sort="datetime";b.requestData.order="desc"}c=Math.ceil(($(window).height()-150)/20);if(c<12)c=12;b.requestData.limit=c;$.wbs.ajaxRequest({url:b.url,data:b.requestData,callback:function(d){b.grid.find("*").remove();b.grid.append(abstractView.create(b.data));
abstractView.renderBlocks(d.requests);b.setFooterData(d);b.grid.find("*").each(function(){$(this).trigger("load")});gridH=$.cookie("grid-height");if(!gridH||gridH>$("#mainPage").height()-100)gridH=Math.ceil($("#mainPage").height()/3);gridH=parseInt(gridH,10);$.cookie("grid-height",gridH);$(".ui-grid-wrapper").height(gridH);$("#container").layout("resizeAll");$this=$(".ui-grid-th-"+b.requestData.sort);$this.addClass("ui-state-active");$this.addClass("ui-grid-order-"+b.requestData.order);typeof a==
"function"&&a();b.cloneHeader();$(".requests-count-div").css("display","inline");$.wbs.widgets.toggleLoading("body","hide");d=$("#first-message");if($("#requests-count").text()=="0"&&d.length){if(d.length){$("#table").hide();d=d.clone().removeAttr("id").show().appendTo($(".ui-grid-wrapper"));var f=$(window).height()-Math.round($(".ui-grid-wrapper").position().top);$.browser.msie?d.css("height",f):d.css("height","100%");d.find(".v-align").css("padding-top","15%");$("#bottomPanel").is(":visible")&&
$.wbs.widgets.toggleBottomPanel(false,true)}else $("#table").show();$.wbs.in_array($.wbs.controller.currentAction,["requestsTrash","requestsVerification"])&&$(".empty-bin-link").remove()}else{d.hide();$("#table").show()}}})},thMouseOver:function(){$(this).addClass("ui-state-highlight ui-grid-th-over")},thMouseOut:function(){$(this).removeClass("ui-state-highlight ui-grid-th-over")},thMouseDown:function(a){if($(a).hasClass("ui-grid-th-id")||$(a).parent().hasClass("ui-grid-th-id")||$(a).hasClass("ui-grid-th-datetime")||
$(a).parent().hasClass("ui-grid-th-datetime")){var b=$(a);a=b.attr("class");var c=/ui-grid-th-(\w+)/.exec(a);b=$("."+c[0]);b.parent().children().removeClass("ui-state-active");b.parent().children().removeClass("ui-grid-order-asc");b.parent().children().removeClass("ui-grid-order-desc");b.addClass("ui-state-active");var d=/ui-grid-order-(\w+)/.exec(a);(sort=c[1])||(sort=$.cookie($.cookie("currentAction")+"-sort"));if(d){b.removeClass("ui-grid-order-"+d[1]);d=d[1]=="desc"?"asc":"desc"}else(d=$.cookie($.cookie("currentAction")+
"-order"))||(d="asc");b.addClass("ui-grid-order-loading");$.cookie($.cookie("currentAction")+"-order",d);$.cookie($.cookie("currentAction")+"-sort",sort);$.extend(gridData.requestData,{sort:c[1],order:d});gridData.updatePage(function(){b.removeClass("ui-grid-order-loading");b.addClass("ui-grid-order-"+d);var f=$("#table tbody tr:first").find(".ui-grid-column-id").attr("rel");$("#grid").wbsGrid("selectBlock",f,true)})}},scrollEvent:function(a){if(!$.wbs.get("appending-rows")){var b=$(a),c=$("table",
b).height()-b.height();b=$("table tr",b).length;c-a.scrollTop-30<c*0.05&&gridData.appendRows(b)}},disableRowClick:false,trMouseDown:function(a){if(!this.disableRowClick)if(!$(a).hasClass("ui-grid-column-new_window")){$(a);var b=$("td",a);$("td.ui-state-highlight",$(a).parent()).removeClass("ui-state-highlight");b.removeClass("ui-state-highlight");b.removeClass("ui-grid-row-over");if(b.hasClass("ui-grid-row-unread")){b.removeClass("ui-grid-row-unread");a=parseInt($.wbs.get("unread_count"),10);if(a>
1){$.wbs.set("unread_count",a-1);$("#myRequestsTitle").css("font-weight","bold");$("#myRequestsTitle span").length||$("#myRequestsTitle").append("<span/>");$("#myRequestsTitle span").html("("+$.wbs.get("unread_count")+")")}else{$.wbs.set("unread_count",0);$("#myRequestsTitle span").remove();$("#myRequestsTitle").css("font-weight","normal")}$("#title h1").text()==$("#myRequestsTitle").text()&&$("#title h1").html($("#myRequestsTitle").text())}b.addClass("ui-state-highlight")}},tdMouseDown:function(a){if($(a).hasClass("ui-grid-column-new_window")){this.disableRowClick=
true;window.open($(a).find("a").attr("rel"))}else{this.disableRowClick=false;a=a.parentNode.tagName=="TR"?$(".ui-grid-column-id",$(a.parentNode)).attr("rel"):$(".ui-grid-column-id",$(a.parentNode).parent()).attr("rel");$.cookie($.cookie("currentAction")+"-id",a);$.wbs.set("current-request-id",a);a=$.wbs.controller.currentHash+"id/"+a;var b=parent?parent.window.location.hash:location.hash;b=="#"+a||b==a?$.wbs.controller.request(a):$.wbs.loadHash(a)}return false},tdOnLoad:function(a){a=$(a);a.hasClass("ui-grid-column-new_window")&&
a.attr("title","[`Open in new window`]");if(a.hasClass("ui-grid-column-state_id")){var b=a.html(),c=$.wbs.get("data").states;if(c[b]&&c[b].name){a.html(c[b].name);c[b].properties&&c[b].properties.css&&a.parent().children().attr("style",c[b].properties.css)}$("#verificationTitle .ui-state-highlight, #trashTitle .ui-state-highlight").length>0&&$("td",a.parent()).attr("style",false)}},data:{gridBody:{data:"",options:{cssClass:"ui-table",css:{"border-collapse":"collapse",width:"100%"},attrs:{id:"table"}}},
spacer:{options:{cssClass:"ui-grid-spacer"}},tableWrapper:{options:{}},headerWrapper:{data:""},headers:{options:{cssClass:"ui-grid-th"},data:[{options:{cssClass:"ui-grid-th-id",css:{cursor:"pointer"}},data:"[`Id`]"},{options:{cssClass:"ui-grid-th-datetime",css:{cursor:"pointer"}},data:"[`Date`]"},{options:{cssClass:"ui-grid-th-client_from"},data:"[`From`]"},{options:{cssClass:"ui-grid-th-subject"},data:"[`Subject`]"},{options:{cssClass:"ui-grid-th-state_id"},data:"[`Status`]"},{options:{cssClass:"ui-grid-th-assigned_c_id"},
data:"[`Assigned`]"},{options:{cssClass:"ui-grid-th-new_window",attrs:{title:"[`Open in new window`]"}},data:"&nbsp;"}]},blockWrapper:{options:{}},blocks:{options:{cssClass:"ui-grid-td"},data:[]},footer:{data:"footer",options:{events:{load:function(){$("#pager",this).pager({pagenumber:gridData.pageNumber,pagecount:gridData.count,buttonClickCallback:function(a){gridData.updatePage(a)}})}}}}}};
$.widget("ui.wbsGrid",{options:{fillHeight:true,width:"100%",model:gridData},loadPage:function(a){this.model.grid=this.element;this.model.loadPage(a)},_create:function(){this.setGridData(this.options.model)},_destroy:function(){this.element.removeData("grid").unbind(".grid")},render:function(){$grid=abstractView.create(this.data);this.element.append($grid);this.element.find("*").each(function(){$(this).trigger("load")})},resize:function(){$(".ui-grid-wrapper:first",this.element).height(this.element.height()-
($.browser.msie?0:$(".ui-table thead tr:first").outerHeight(true)))},setSort:function(){},getCurrentPage:function(){return $("li.pgCurrent").text()},selectBlock:function(a,b){var c=[];$rows=this.element.find("tr").find(".ui-grid-column-id");$rows.each(function(){if($(this).attr("rel")==a)c=$(this).parent()});if(a>0){$("#textarea").wbsEditor("loadContent",{ajax:true,url:"?m=requests&act=info&id="+a});$.wbs.set("current-request-id",a)}else{c=this.element.find("tbody tr:first");a=$(".ui-grid-column-id",
c).attr("rel");if(c.length){$("#textarea").wbsEditor("loadContent",{ajax:true,url:"?m=requests&act=info&id="+a});$.wbs.set("current-request-id",a)}}if(c.length){c.mousedown();/\/plugin\/(\w+)/.exec(location.hash);$.wbs.set("current-request-id",$(".ui-grid-column-id",c).attr("rel"));if(b){var d=c.prev().prev();d.length?c.closest(".ui-grid-wrapper").scrollTo(d):c.closest(".ui-grid-wrapper").scrollTo(0)}}return false},setGridData:function(a){var b=this;gridData.grid=this.element;gridData.loadPage(function(){b.selectBlock($.wbs.get("current-request-id"),
true);typeof a=="function"&&a()})}});(function(a){a.widget("ui.layout",{options:{hiddenPanels:[],hiddenElements:[],resizeTimeout:500,callback:false,onResize:function(){return false}},_parseClassName:function(b){b=b.split(":");return{name:b[0],value:b[1]}},_applyAttrs:function(b){for(key in attr)b.addClass(key+":"+attr[key]);return b},_buildDom:function(b){var c=a("<div class='ui-layout-panel'></div>");c.attr("id",b.id);return c},_parsePanel:function(b){var c=b.attr("class").split(" "),d=[];d.id=b.attr("id").replace(/-markup/,"");for(classEl in c){c[classEl]=
a.trim(c[classEl]);if(c[classEl]!=""){b=this._parseClassName(c[classEl]);d[b.name]=b.value}}return d},clsSel:function(b,c){return"."+b+"\\:"+c},createPanelTree:function(b,c){for(var d=0;d<b.length;d++){var f=a(b[d]);f.attr=this._parsePanel(f);c[f.attr.id]={};c[f.attr.id].attr=f.attr;var e=f.children();e.length>0&&this.createPanelTree(e,c[f.attr.id])}return c},_cssValParse:function(b,c){if(b){b=b.replace(/medium/,"1");if(b.indexOf("%")>0){b=b.replace(/\%/g,"");b=Math.round(c*(b/100))}else b=b.replace(/px/g,
"");return parseInt(b,10)}else return 0},_getContentSize:function(b){return{width:b.outerWidth(),height:b.outerHeight()}},getMaxChildrenSize:function(b){b.parent();var c={width:0,height:0};b.children().each(function(){var d=a(this);if(!d.hasClass("ui-layout-panel")){d.rightPoint=d.outerWidth(true)+d.position().left;if(c.width<d.rightPoint)c.width=d.rightPoint;d.bottomPoint=d.outerHeight(true)+d.position().top;if(c.height<d.bottomPoint)c.height=d.bottomPoint}});(b=b.position())||(b={left:0,top:0});
c.width=Math.floor(c.width-b.left+2);c.height=Math.floor(c.height-b.top);return c},getOverallChildrenSize:function(b){var c={width:0,height:0};b.children().children().each(function(){var d=a(this);c.width+=d.outerWidth(true)});return c},getEmptySpace:function(b){var c=this,d=b.parent(),f={width:0,height:0},e=d.children(),g=a.wbs.getObjByKey(d.attr("id"),this.layout,true),h=a.wbs.getObjByKey(b.attr("id"),c.layout,true),i=h.width,k=h.height,j=!(i&&i.search("%")),m=!(k&&k.search("%"));e.each(function(){var l=
a(this);l.node=a.wbs.getObjByKey(l.attr("id"),c.layout,true);l.curWidth=h.width;l.curHeight=h.height;if(l.is(":visible"))if(l.not(b).length&&l.hasClass("ui-layout-panel")){if(g&&g.stack=="h")if(l.curWidth&&l.curWidth.search("%")){if(j)f.width+=l.outerWidth()}else f.width+=l.outerWidth();if(g&&g.stack!="h")if(l.curHeight&&l.curHeight.search("%")){if(m)f.height+=l.outerHeight()}else f.height+=l.outerHeight()}});d.widthVal=d.width()-f.width;d.heightVal=d.height()-f.height;return{width:d.widthVal,height:d.heightVal,
siblingsSize:f}},getEmptySpaceBorders:function(b){var c=b.parent(),d={left:0,right:c.position().left+c.outerWidth(true)};c=c.children();var f=d.right;c.each(function(){var e=a(this);if(e.not(b).length&&e.hasClass("ui-layout-panel")){e.leftPoint=e.position().left;e.rightPoint=e.outerWidth(true)+e.position().left;if(d.left<e.rightPoint&&e.rightPoint!=f)d.left=e.rightPoint;if(d.right>e.leftPoint&&e.rightPoint==f)d.right=e.leftPoint}});return{left:d.left,right:d.right}},_applyAttr:function(b,c){var d=
b.value.attr,f=a.wbs.getById(b.id,a("#"+c.id)),e={},g=a("#"+c.id);for(attrKey in d){var h=d[attrKey];switch(attrKey){case "overflow":e.overflow=h;break;case "widget":a.wbs.widgets[h]&&a.wbs.widgets[h](f);for(var i in this.options.hiddenElements)a("#"+this.options.hiddenElements[i]).hide();break;case "heightConstraint":case "widthConstraint":i=attrKey.replace(/Constraint/,"");h=d[attrKey].split("-");for(valKey in h)h[valKey]=this._cssValParse(h[valKey],g[i]());e["min-"+i]=h[0]}}if(b.id=="grid")e.overflow=
"hidden";e&&f.css(e);if(d.nowrap){d=a("<div/>");f.children().wrapAll(d)}},_append:function(b,c){var d=this._buildDom(b.value.attr);a.wbs.getById(c.id,this.element).append(d);c.attr&&!c.attr.stack&&a.wbs.getById(c.id,this.element).append('<div class="ui-layout-clear"></div>')},_setInnerSize:function(b,c,d){if(d){d=d-this._cssValParse(b.css("border-left-width"))-this._cssValParse(b.css("border-right-width"))-this._cssValParse(b.css("padding-left"))-this._cssValParse(b.css("padding-right"));b.css(c,
d)}},_resize:function(b,c){var d=b.value.attr,f={width:false,height:false},e=a.wbs.getById(b.id,a("#"+c.id));a("#"+c.id);var g=this.getEmptySpace(e);switch(d.fill){case "none":f.width=d.width;f.height=d.height;break;case "all":f.width=g.width;f.height=g.height;break;case "h":f.width=g.width;if(d.height)f.height=this._cssValParse(d.height,g.height);break;case "v":f.height=g.height;if(d.width)f.width=this._cssValParse(d.width,g.width)}if(d.heightOffset)f.height=parseInt(g.height)+parseInt(d.heightOffset);
f.width||delete f.width;f.height||delete f.height;b.id=="gridPanel"&&!a("#bottomPanel").is(":visible")?e.css({width:"100%",height:a("#mainPage").height()-a("#toolbar").height()}):e.css(f)},treeTraversal:function(b,c,d){this[d](b,c);for(var f in b.value)f!="attr"&&this.treeTraversal({id:f,value:b.value[f]},{id:b.id,value:b.value},d)},create:function(){this._create()},resizeAll:function(){this.treeTraversal(a.wbs.getFirstChild(this.layout),this.layout,"_resize");this.options.onResize()},_create:function(){var b=
this.options;this.element.addClass("ui-layout");if(!b.layoutMarkupId)b.layoutMarkupId=this.element.attr("id");this.markup=a("#"+b.layoutMarkupId,a(".ui-layout-markup"));this.markup.attr("id",this.markup.attr("id")+"-markup");this.markup.find("*").each(function(){this.id+="-markup"});this.layout=this.createPanelTree(this.markup,[]);var c=a.wbs.getFirstChild(this.layout);this.layout.id=this.element.attr("id");this.treeTraversal(c,this.layout,"_append");this.treeTraversal(c,this.layout,"_applyAttr");
typeof b.callback=="function"&&b.callback();this.resizeAll()}})})(jQuery);