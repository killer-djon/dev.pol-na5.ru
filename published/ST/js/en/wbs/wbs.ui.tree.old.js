...
$.widget("ui.tree", {
	...
	_setNodeEvents : function(el) {
		...
		//for (var i = 0; i < droppable.length; i++) {
			...
			var elements = $(droppable[0].element, this.element);
			if (elements.length) {
				var options = this._createDDNodeOptions(droppable[0], 'drop');
				if ($.ui.droppable) $(elements).droppable(options).data('tree' , this);
			}
		//}
		for (var i = 0; i < draggable.length; i++) {
			...
			if (elements.length) {
				...
				if ($.ui.draggable) elements.draggable(options).data('tree' , this);
			}
		}
	...
	_createLI: function(obj) {
		...
		html += '"><div class="ui-tree-node-header ' + emptyClass + '">';
		
		if (obj.title) {
			if (obj.url) html += '<a href="' + obj.url + '"><span class="ui-tree-node-text ui-state-default">' + obj.title + '</span></a>';
			else html += '<a rel=""><span class="ui-tree-node-text ui-corner-all ui-state-default">' + obj.title + '</span></a>';
		}
		html += '</div>';
		if (obj.children) {
...
var $_ui_tree_defaults = {
	...
	draggable : {
		element : '*',
		//handle : 'span.ui-state-highlight',
		helper : 'clone',
		revert : 'invalid',
		distance: 2
	},
	...
	drop : function(event, ui) {
		...
		var json = ui.sender ? ui.sender.getJSON(ui.draggable) : "{title : '" + $.trim(ui.draggable.text()) + "'}";

		ui.target.append(json, ui.droppable);
		$(ui.draggable).remove();
		var parent = $(ui.droppable).parent();
		var parent2 = $(ui.sender.element);
		$('.ui-draggable-dragging').remove();
		//	console.log($('>li:last',parent));
		$('>li',parent2).removeClass('ui-tree-node-last-visible');
		$('>li:first',parent2).addClass('ui-tree-node-first-visible');
		$('>li:last',parent2).addClass('ui-tree-node-last-visible');
		
		$('>li',parent).removeClass('ui-tree-node-last-visible');
		$('>li:first',parent).addClass('ui-tree-node-first-visible');
		$('>li:last',parent).addClass('ui-tree-node-last-visible');
	},
