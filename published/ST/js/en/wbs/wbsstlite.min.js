...
m.wbsPopup({padding:3,parent:j,absolute:true})});break;case "link":g.data("onLoad",function(j){j.wrap('<div style="position: relative;float:left;"></div>');var m=$("<div/>"),l=$("<input value='http://' class='disable-selection-on-focus' style='width:150px' />"),o=$("<button>OK</button>").click(function(){f.doc.execCommand("createlink",false,l.val());m.wbsPopup("close");f.textarea.val(f.editablefield.html());return false});m.append("Enter link address:").append(l).append(o);m.wbsPopup({padding:3,
...
g).val($(this).val())});$dialog.dialog({modal:true,title:"Change classifiers",zIndex:1E4,open:function(g){$parent=$(g.target).parent();var h=$parent.width();g=Math.round($(window).width()/2);h=(g-h)/2;$parent.width(g);$parent.offset({left:$parent.offset().left-h})}})}})}};dragged=false;
