...
$().ajaxComplete(function(request, settings){
	if ( settings.responseText.match(/Allowed memory/) ) {
		alert($.sprintf('Not enough memory to perfom operation (memory_limit=%s)', document.memory_limit));
	}
});
...
//				var $addPanel = $('<div id="album-add-panel"> </div>');
				if ( Rights.manage_album == 1 ) {
					$addPanel.append('<div class="albom-manage"><a href="#" id="btn-album-arrange">Organize albums</a></div>');
				}
				if ( Rights.create_album == 1 ) {
					$addPanel.append('<input value="New album" id="btn-album-add" type="button" />');
				}
				$addPanel.append("<span id=\"album-arrange-tile\" style=\"display: none;\">Drag'n'drop to arrange albums.</span>");
//				alert('4');
...
TemplateAlbums = {
	render: function(elem, data) {
		...
		if ( this.data.length == 0 ) {
			$('<div class="center">There are no photo albums in your library.<br /><br /><em>Create a new album to upload photos.</em> </div>')
				.appendTo($ulContener);
			return;
		}
	...
	_createAlbumItem: function(record) {
		...
		if ( record.RIGHT >= 1 ) {
			...
			var title = 'You do not have permissions to access this album.';
		}
		...
		if (record.PF_STATUS == 1) {
			$divpublished.append('<span class="publish_yes" title="Album is published"> </span>');
		}
		if (record.PF_STATUS == 3) {
			$divpublished.append('<span class="publish_hide" title="Album is published privately"> </span>');
		}
		...
		if ( record.RIGHT > 1 ) {
			$div.append( $('<a href="#" class="delete-album" style="display: none;">Delete</a>').click(function(res){
				if ( confirm( 'This will permanently delete all photos inside this album. Delete?' ) ) {
...
//------------------------			
			case StateName.ImageTile : {
				...
				$('<a href="#" class="iconlink" id="slideshow-link"><span class="icon"></span><b>Slideshow</b></a>').click(function(){
	            ...
	            if (  Rights.isWrite() ) {
	            	if (album.PF_DESC.length > 0)
						$desc.append('<a id="album-desc-edit" href="#"><span></span>edit</a>')
					else
						$desc.append('<a id="album-desc-edit" href="#"><span></span>Edit album description</a>')
	            }
	             
	            if (  Rights.isWrite() ) {
					var $pl1 = $('<p class="panel-link" > </p>');
					var $pl2 = $('<p class="panel-link" > </p>');
					var $view = $('<a id="view-link" class="opened" href="#"><span></span><span class="dott-border">View</span></a>');
					var $arrange = $('<a id="arrange-link" class="closed" href="#"><span></span><span class="dott-border">Organize and share</span></a>');
				...
				$arrangeUtils.append("<p>Arrange photos using drag'n'drop.</p><br>");
				$arrangeUtils.append('<p>With selected photos (<span class="selectCount">0</span>):</p>');
				
				$('<p class="selectall"><a href="#">select all</a><p>').appendTo($arrangeUtils).click(function(){
				...
				$('<a class="albumlinks" href="#"><span></span><b>Links to photos</b></a>').appendTo($arrangeUtils).click(function(){
					var images = [];
				...
				if ( Rights.manage_collections ) {
					$arrangeUtils.append('<a class="createlink" href="#"><span></span><b>Create a collection from selected photos</b></a>');
				}
				
				$('<a class="transfer" href="#"><span></span><b>Move to another album</b></a>').appendTo($arrangeUtils).click(function(){					
				...
				var $deleteSelect = $('<a class="albumdelete" href="#"><span></span><b>Delete photos</b></a>');
				...
				if ( Rights.isWrite() ) {
					...
					$('<a href="#" id="descr-edit" class="iconlink"><span class="icon"></span><b>Edit photo descriptions</b></a>').click(function(){
						$('#pp').wbsPopup({
					...
					if ( Rights.manage_collections ) {
						...
						var $links = $('<a id="creat-collection" class="iconlink" href="#"><span class="icon"> </span><b>Create a collection</b></a>');				
						$block3.append($links);
					...
					var $flickr = $('<a id="flickr-import" href="#" class="iconlink"><span class="icon"></span><b>Import from Flickr<img src="img/new_window_icon.gif" width="13" height="11" alt="" /></b></a>');
...
//						});
					...
					if ( Rights.isFull() )
					$('<a id="delete-album" href="#" class="iconlink"><span class="icon"></span><b>Delete album</b></a>')
						.click(function(){
...
//------------------------			
			case StateName.ImageOne : {
				...
				if ( Rights.isWrite() ) {
					if ( record.PL_DESC.length > 0 )
						$desc.append('<a href="#">edit</a>');
					else 
						$desc.append('<a href="#">Edit photo description</a>');
				...
				$photourls.append('<a id="link_main_a" href="" target="_blank"><img src="img/new_window_icon.gif" /></a><label for="link_main"><b>Link (for email and IM):</b></label>');
				$photourls.append('<p><input id="link_main" type="text" value="" readonly="true" /> </p>');
				
				$photourls.append('<label for="link_html">HTML (for websites and blogs):</label>');
				$photourls.append('<p><input id="link_html" type="text" value="" readonly="true" /> </p>');
				
				$photourls.append('<label for="link_bb">BBcode (forums):</label>');
				...
				$block1_5.append('<h4>Original</h4>');
				...
				if ( Rights.isWrite() ) {
						...
						var $leftRotate = $('<a href="#" class="sidebar-btn rleft"><span class="active" alt="Rotate left" title="Rotate left" >&nbsp;</span></a>');
						...
						var $rigthRotate = $('<a href="#" class="sidebar-btn rright"><span class="active" alt="Rotate right" title="Rotate right">&nbsp;</span></a>');
		...
		//				$cropdiv = $('<div class="rigth-panel-crop" />');
						if ( $.cookie('sizeOneView') <= 144 )
							...
							var hide = '';
						var $cropbtn = $('<a href="#" class="sidebar-btn crop" '+hide+'><span class="active" alt="Crop" title="Crop">&nbsp;</span></a>');
						...
						var $cropDoneBtn = $('<input class="sidebar-btn crop-done" value="Crop" type="button"/>');
...
//						});
						...
						var $cropCancelBtn = $('<a href="#" class="sidebar-btn crop-cancel" >Cancel</a>');
		...
		//				var $block5 = $('<div class="rigth-panel-block albumface"> </div>');
						...
						if (this.album.PF_THUMB != this.record.PL_ID) {
							var st = '<b>Make this a key photo</b>';
							...
							var st = $.sprintf('This is a key photo for this album "%s"', name);
						...
						$('<a href="#" class="'+class_+'"><span> </span>'+st+'</a>').click(function(){					
							$.post('backend.php?controller=ajax&action=changeThumbAlbum', {
								...
								if ( data.status == "OK" ) {									
									...
									$('.thumb_btn').html($.sprintf('<span> </span><b class="lightb">This is a key photo for album "%s".</b>', name)).removeClass().addClass('thumb_btn_des');
								}							}, "json");				
							return false;
				...
				if ( Rights.isWrite() ) {		
					...
					$('<a id="delete-image" href="#" class="iconlink"><span class="icon"></span><b>Delete photo</b></a>')
						.click(function(){
	
							if ( confirm( 'This will permanently delete photo. Are you sure?' ) ) {
				...
				$block3.append('<p class="exif-info"><b>Details (EXIF)</b><p>');
	
				if ( exif.PE_DATETIME && exif.PE_DATETIME.length > 0 ) {
					$block3.append('<p>Taken: <span>'+exif.PE_DATETIME+'</span></p>');
				}
				if ( this.record.PL_UPLOADDATETIME && this.record.PL_UPLOADDATETIME.length > 0 ) {
					$block3.append('<p>Uploaded: <span>'+this.record.PL_UPLOADDATETIME+'</span></p>');
				}

				if ( this.record.PL_W || exif.PL_H ) {
					$block3.append('<p>Image size: <span>'+parseInt(this.record.PL_W)+'x'+parseInt(this.record.PL_H)+'</span></p>');
				}
				
				if ( exif.PE_MODEL && exif.PE_MODEL.length > 0 ) {
					$block3.append('<p>Camera: <span>'+exif.PE_MODEL+'</span></p>');
				}
				if ( exif.PE_EXPOSURETIME && exif.PE_EXPOSURETIME.length > 0 ) {
					var expo = ( /1\//.test(exif.PE_EXPOSURETIME) ) ? exif.PE_EXPOSURETIME : exif.PE_EXPOSURETIME + ' sec' ;
					$block3.append('<p>Shutter: <span>'+expo+'</span></p>');
				}
				if ( exif.PE_FNUMBER && exif.PE_FNUMBER.length > 0 ) {
					$block3.append('<p>Aperture: <span>'+exif.PE_FNUMBER+'</span></p>');
				}
				if ( exif.PE_FOCALLENGTH && exif.PE_FOCALLENGTH.length > 0 ) {
					$block3.append('<p>Focal length: <span>'+exif.PE_FOCALLENGTH+'</span></p>');
				}
		...
		switch ( state.stateName ) {
			case StateName.Alubms : {
				...
				jQuery('#btn-album-arrange').toggle(
					function(){
						...
						jQuery('#btn-album-arrange').text("View albums mode");
					...
					function(){
						jQuery('#btn-album-arrange').text("Organize albums");
			...
			case StateName.ImageTile : {
				...
				$('#album-name').text( albumName );		
				 ...
				 if (  Rights.isWrite() ) {
					var $rename = $('<span class="album-rename">rename</span>').click(function(){ 
						$(this).hide();
						$('#album-name').click() 
					});
				...
				if ( Rights.isWrite() ) {
					...
					.editable("backend.php?controller=ajax&action=cangeAlbumName", { 
						...
						endEvent: function(){
							WbsData.set({'isKeyEnable' : true});
//							$('#album-name').html($('#album-name').text() + '<span class="album-rename">rename</span>');
							$('.album-rename').show();
						},
				...
				if ( frontLink.length > 45 ) {
					...
					if (album.PF_STATUS == 0 || album.PF_STATUS ==2) {
						$('#album-access').get(0).innerHTML = "<span class='publish-no'>Album is not published.</span>";
						$('#album-url').empty();
						
						if ( Rights.isFull() ) 							
						$('#btn-album-setting').text('Publish');
					}
					else if(album.PF_STATUS == 1) {
						$('#album-access').get(0).innerHTML = "<span class='publish-yes'>Album is published:</span>";
						$('#album-url').empty().append( '<a target="_blank" href="'+document.mainUrl+album.PF_LINK+'">'+frontLink+' <img src="img/new_window_icon.gif" /></a>');
						
						if ( Rights.isFull() )
						$('#btn-album-setting').text('Settings');
					}
					else if(album.PF_STATUS == 3) {
						$('#album-access').get(0).innerHTML = "<span class='publish-lock'>Album is published privately:</span>";
						$('#album-url').empty().append( $('<a target="_blank" href="'+document.mainUrl+album.PF_LINK+'">'+frontLink+' <img src="img/new_window_icon.gif" /></a>'));
...
//					imageCount.innerHTML = data.imageCount+' photos';
...
//				this.contentElem = createDiv("");
				...
				$('<a href="#" title="Back to albums"><img style="width: 25px; height: 25px" src="img/back.gif" alt="Back to albums"/>Albums</a>').appendTo($back_button).click(function(){
				...
				$(this.contentElem).append('<div class="image-count"><span class="image-page-count">'+limit+'</span>photos on a page</div>');
...
//				this.contentElem = createDiv("");
				...
				$('<div class="back-to-album"><img src="img/back.gif" width="25" height="25" alt="Back to album" /><a href="#">Back to album</a></div>').appendTo(this.contentElem).find('a').click(function(){
				...
				span.innerHTML = $.sprintf('Photo %s of %s', parseInt(this.data.PL_SORT) + 1, data.imageCount);
...
WaArrange = newClass(null, {
	constructor: function (config) {
		...
		for(var i=0; i<this.listElements.length; i++) {
			...
			$(this.listElements[i]).find('.image-contener').attr('title', "Click to select, drag'n'drop to sort");
...
WaPager = newClass (null, {
	...
	render: function(state, data){
		...
		if (data.total == 0) {
			if( Helper.getManager().getState().stateName == StateName.Alubms )
				div.innerHTML = "<div class='page-button-div'>Create a new album to upload photos.</div>";
			else
				div.innerHTML = "<div class='page-button-div'>There are no photos in this album.</div>";
		} else		
		if( Helper.getManager().getState().stateName == StateName.Alubms ) {
			div.innerHTML = "<div class='page-button-div'>"+$.sprintf("%d photos in %d album(s)",data.imageCount,data.total)+"</div>";
		}
		else
			div.innerHTML = "<div class='page-button-div'>Photos<span>" +start+"</span>&mdash;<span>"+ end +"</span> of <span>"+data.total+"</span></div>";
...
(function($) {
	$.fn.viewModeSelector = function (param) {
		...
		var $vmSelector = $('<div class="viewmode-select"><span>View:</span></div>');
