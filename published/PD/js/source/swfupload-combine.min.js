...
function fileQueueError(a,b,c){try{if(b===SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED)alert($.sprintf("[`You have attempted to queue too many files.`]\n[`You may select up to %s files.`]",c));else{var d=new FileProgress(a,this.customSettings.progressTarget);d.setError();d.toggleCancel(false);switch(b){case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:d.setStatus("[`File size exceeds maximum allowed by the server.`]");this.debug("Error Code: File too big, File name: "+a.name+", File size: "+a.size+
", Message: "+c);break;case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:d.setStatus("[`Cannot upload 0 byte files.`]");this.debug("Error Code: Zero byte file, File name: "+a.name+", File size: "+a.size+", Message: "+c);break;case SWFUpload.QUEUE_ERROR.INVALID_FILETYPE:d.setStatus("[`Invalid File Type.`]");this.debug("Error Code: Invalid File Type, File name: "+a.name+", File size: "+a.size+", Message: "+c);break;default:a!==null&&d.setStatus("[`Error`]");this.debug("Error Code: "+b+", File name: "+a.name+
", File size: "+a.size+", Message: "+c);break}}}catch(e){this.debug(e)}}
function fileDialogComplete(a){try{if(a>0){document.getElementById(this.customSettings.cancelButtonId).disabled=false;$(".upload-button").hide();$("#SWFUpload_0").css({position:"absolute",left:0,top:0});if(limit!=-100&&limit<FileInfo.fileSize){$(".error-view").text($.sprintf("[`You have only %s of available disk space left. Not all photos will be uploaded.`]",$.filesizeformat(limit)));$(".error-view").css({color:"red"})}FileInfo.updateView()}}catch(b){this.debug(b)}}
function uploadStart(a){try{var b=new FileProgress(a,this.customSettings.progressTarget);b.setStatus("[`Uploading...`]");b.toggleCancel(true,this)}catch(c){}return true}
function uploadProgress(a,b,c){try{var d=Math.ceil(b/c*100),e=new FileProgress(a,this.customSettings.progressTarget);e.setProgress(d);if(d==100){e.setStatus("[`Uploading... Creating thumbnails...`]");$(".progressContainer.green > .progressBarStatus").append('<img src="img/loader1.gif" />')}var f=FileInfo.uploadSize+b,g=f/FileInfo.fileSize;$("#progressBarStripe").css({left:g*200});$(".progressValue").text(Math.ceil(parseInt(g*100))+"%");$("#divStatus").text($.filesizeformat(f)+" [`of`] "+$.filesizeformat(FileInfo.fileSize))}catch(h){this.debug(h)}}
function uploadSuccess(a,b){try{FileInfo.uploadFile(a);if(b.match(/Allowed memory/i)){var c=new FileProgress(a,this.customSettings.progressTarget);c.setError();c.setStatus($.sprintf("[`Not enough memory to perfom operation (memory_limit=%s)`]",memory_limit));c.toggleCancel(false);FileInfo.errorAdd()}else{if(b.match(/File type not supported/i)){c=new FileProgress(a,this.customSettings.progressTarget);c.setError();c.setStatus("[`Invalid file type.`]")}else{c=new FileProgress(a,this.customSettings.progressTarget);
c.setComplete();c.setStatus("[`Complete.`]")}c.toggleCancel(false)}this.startUpload()}catch(d){this.debug(d)}}
function uploadError(a,b,c){try{var d=new FileProgress(a,this.customSettings.progressTarget);d.setError();if(a.filestatus==-5){FileInfo.deleteFile(a);FileInfo.updateView()}switch(b){case SWFUpload.UPLOAD_ERROR.HTTP_ERROR:if(c=="500"||b=="500")d.setStatus("[`Photo was not uploaded. Not enough permissions or out of disk space.`]: [`Photo was not uploaded. Not enough permissions or out of disk space.`]");c=="401"&&d.setStatus("[`403 Forbidden`]");this.debug("Error Code: HTTP Error, File name: "+a.name+
", Message: "+c);break;case SWFUpload.UPLOAD_ERROR.UPLOAD_FAILED:d.setStatus("[`Upload Failed.`]");this.debug("Error Code: Upload Failed, File name: "+a.name+", File size: "+a.size+", Message: "+c);break;case SWFUpload.UPLOAD_ERROR.IO_ERROR:d.setStatus("[`Server (IO) Error`]");this.debug("Error Code: IO Error, File name: "+a.name+", Message: "+c);break;case SWFUpload.UPLOAD_ERROR.SECURITY_ERROR:d.setStatus("[`Security Error`]");this.debug("Error Code: Security Error, File name: "+a.name+", Message: "+
c);break;case SWFUpload.UPLOAD_ERROR.UPLOAD_LIMIT_EXCEEDED:d.setStatus("[`Upload limit exceeded.`]");this.debug("Error Code: Upload Limit Exceeded, File name: "+a.name+", File size: "+a.size+", Message: "+c);break;case SWFUpload.UPLOAD_ERROR.FILE_VALIDATION_FAILED:d.setStatus("[`Failed Validation.  Upload skipped.`]");this.debug("Error Code: File Validation Failed, File name: "+a.name+", File size: "+a.size+", Message: "+c);break;case SWFUpload.UPLOAD_ERROR.FILE_CANCELLED:if(this.getStats().files_queued===
0)document.getElementById(this.customSettings.cancelButtonId).disabled=true;d.setStatus("[`Cancelled`]");d.setCancelled();break;case SWFUpload.UPLOAD_ERROR.UPLOAD_STOPPED:d.setStatus("[`Stopped`]");break;default:d.setStatus("[`Error creating thumbnail (possible reason \u2014 not enough memory).`]");this.debug("Error Code: "+b+", File name: "+a.name+", File size: "+a.size+", Message: "+c);break}}catch(e){this.debug(e)}}
