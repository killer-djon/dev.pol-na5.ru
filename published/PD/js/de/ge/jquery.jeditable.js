...
(function($) {
    ...
    $.fn.editable = function(target, options) {
        ...
        var startEvent = settings.startEvent || function() { };
        var endEvent = settings.endEvent || function() { };
        var cancelEvent = settings.cancelEvent || function() { };
        ...
        return this.each(function() {
            ...
            $(this)[settings.event](function(e) {
                ...
                self.editing    = true;
                self.revert     = $(self).next().text();
                self.revertmin  = $(self).text();
...
//                    endEvent.apply(self, [self.innerHTML, settings]);
                    ...
                    if (false !== submit.apply(form, [settings, self])) { 
                      ...
                      if ($.isFunction(settings.target)) {
                          ...
                          $.post(settings.target, submitdata, function(str) {                          	  
                          	  var result = str.response;
                          	  if(result.length <= 0) return false;
                          	  $(self).next().text(result);
                          	  if ( result.length > settings.fieldCount ) {
								  result = result.substr(0, settings.fieldCount-3) + '...';
							  }                          	
                              $(self).text(result);
                              $(self).attr('title', result);
                              self.editing = false;
                              callback.apply(self, [self.innerHTML, settings]);
                              /* TODO: this is not dry */                              
                              if (!$.trim($(self).html())) {
            ...
            defaults: {
                ...
                buttons : function(settings, original) {
                    ...
                    if (settings.submit) {
                        ...
                        $(this).append(submit);      
                        $(this).append(' '+settings.postSubmitText+' '); 
                    }
                    if (settings.cancel) {
                        /* if given html string use that */
