...
function fileQueued(a){try{FileInfo.addFile(a);var b=new FileProgress(a,this.customSettings.progressTarget);b.setStatus($.filesizeformat(a.size));b.toggleCancel(true,this)}catch(c){this.debug(c)}}
function fileQueueError(a,b,c){try{if(b===SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED)alert($.sprintf("Вы выбрали слишком много файлов.\nМожно выбрать не более %s файлов.",c));else{var d=new FileProgress(a,this.customSettings.progressTarget);d.setError();d.toggleCancel(false);switch(b){case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:d.setStatus("Размер файла превышает максимально допустимый сервером.");this.debug("Error Code: File too big, File name: "+a.name+", File size: "+a.size+
", Message: "+c);break;case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:d.setStatus("Невозможно загрузить файл с нулевым размером.");this.debug("Error Code: Zero byte file, File name: "+a.name+", File size: "+a.size+", Message: "+c);break;case SWFUpload.QUEUE_ERROR.INVALID_FILETYPE:d.setStatus("Недопустимый тип файла.");this.debug("Error Code: Invalid File Type, File name: "+a.name+", File size: "+a.size+", Message: "+c);break;default:a!==null&&d.setStatus("Ошибка");this.debug("Error Code: "+b+", File name: "+a.name+
", File size: "+a.size+", Message: "+c);break}}}catch(e){this.debug(e)}}
function fileDialogComplete(a){try{if(a>0){document.getElementById(this.customSettings.cancelButtonId).disabled=false;$(".upload-button").hide();$("#SWFUpload_0").css({position:"absolute",left:0,top:0});if(limit!=-100&&limit<FileInfo.fileSize){$(".error-view").text($.sprintf("У вас осталось %s доступного места на диске. Не все фотографии будут загружены.",$.filesizeformat(limit)));$(".error-view").css({color:"red"})}FileInfo.updateView()}}catch(b){this.debug(b)}}
function uploadStart(a){try{var b=new FileProgress(a,this.customSettings.progressTarget);b.setStatus("Загрузка...");b.toggleCancel(true,this)}catch(c){}return true}
function uploadProgress(a,b,c){try{var d=Math.ceil(b/c*100),e=new FileProgress(a,this.customSettings.progressTarget);e.setProgress(d);if(d==100){e.setStatus("Загрузка... Создание эскизов...");$(".progressContainer.green > .progressBarStatus").append('<img src="img/loader1.gif" />')}var f=FileInfo.uploadSize+b,g=f/FileInfo.fileSize;$("#progressBarStripe").css({left:g*200});$(".progressValue").text(Math.ceil(parseInt(g*100))+"%");$("#divStatus").text($.filesizeformat(f)+" из "+$.filesizeformat(FileInfo.fileSize))}catch(h){this.debug(h)}}
function uploadSuccess(a,b){try{FileInfo.uploadFile(a);if(b.match(/Allowed memory/i)){var c=new FileProgress(a,this.customSettings.progressTarget);c.setError();c.setStatus($.sprintf("Недостаточно памяти для выполнения операции (memory_limit=%s)",memory_limit));c.toggleCancel(false);FileInfo.errorAdd()}else{if(b.match(/File type not supported/i)){c=new FileProgress(a,this.customSettings.progressTarget);c.setError();c.setStatus("Неверный тип файла.")}else{c=new FileProgress(a,this.customSettings.progressTarget);
c.setComplete();c.setStatus("Готово.")}c.toggleCancel(false)}this.startUpload()}catch(d){this.debug(d)}}
function uploadError(a,b,c){try{var d=new FileProgress(a,this.customSettings.progressTarget);d.setError();if(a.filestatus==-5){FileInfo.deleteFile(a);FileInfo.updateView()}switch(b){case SWFUpload.UPLOAD_ERROR.HTTP_ERROR:if(c=="500"||b=="500")d.setStatus("Фотография не загружена на сервер. Вероятно, нет прав для загрузки или недостаточно места на диске.: Фотография не загружена на сервер. Вероятно, нет прав для загрузки или недостаточно места на диске.");c=="401"&&d.setStatus("403 Forbidden");this.debug("Error Code: HTTP Error, File name: "+a.name+
", Message: "+c);break;case SWFUpload.UPLOAD_ERROR.UPLOAD_FAILED:d.setStatus("Файл не был загружен.");this.debug("Error Code: Upload Failed, File name: "+a.name+", File size: "+a.size+", Message: "+c);break;case SWFUpload.UPLOAD_ERROR.IO_ERROR:d.setStatus("Ошибка доступа к серверу (IO)");this.debug("Error Code: IO Error, File name: "+a.name+", Message: "+c);break;case SWFUpload.UPLOAD_ERROR.SECURITY_ERROR:d.setStatus("Ошибка загрузки файла (security error).");this.debug("Error Code: Security Error, File name: "+a.name+", Message: "+
c);break;case SWFUpload.UPLOAD_ERROR.UPLOAD_LIMIT_EXCEEDED:d.setStatus("Превышено ограничение на загрузку.");this.debug("Error Code: Upload Limit Exceeded, File name: "+a.name+", File size: "+a.size+", Message: "+c);break;case SWFUpload.UPLOAD_ERROR.FILE_VALIDATION_FAILED:d.setStatus("Файл пропущен.");this.debug("Error Code: File Validation Failed, File name: "+a.name+", File size: "+a.size+", Message: "+c);break;case SWFUpload.UPLOAD_ERROR.FILE_CANCELLED:if(this.getStats().files_queued===
0)document.getElementById(this.customSettings.cancelButtonId).disabled=true;d.setStatus("Загрузка отменена");d.setCancelled();break;case SWFUpload.UPLOAD_ERROR.UPLOAD_STOPPED:d.setStatus("Загрузка остановлена");break;default:d.setStatus("Error creating thumbnail (possible reason \u2014 not enough memory).");this.debug("Error Code: "+b+", File name: "+a.name+", File size: "+a.size+", Message: "+c);break}}catch(e){this.debug(e)}}
function uploadComplete(){if(this.getStats().files_queued===0)document.getElementById(this.customSettings.cancelButtonId).disabled=true}function queueComplete(a){FileInfo.stopUpload();window.parent.refreshToUpload(FileInfo.uploadCount);FileInfo.reset();FileInfo.updateView();document.getElementById("divStatus").innerHTML=a+" file"+(a===1?"":"s")+" uploaded.";$("#fsUploadProgress").offset();$("#SWFUpload_0").css({position:"absolute",left:130,top:50})}
...
FileProgress.prototype.setComplete=function(){this.fileProgressElement.className="progressContainer blue";this.fileProgressElement.childNodes[3].className="progressBarComplete";this.fileProgressElement.childNodes[3].style.width="";var a=this;this.setTimer(setTimeout(function(){a.disappear()},1E4))};
...
FileProgress.prototype.toggleCancel=function(a,b){this.fileProgressElement.childNodes[0].style.visibility=a?"visible":"hidden";if(b){var c=this.fileProgressID;this.fileProgressElement.childNodes[0].onclick=function(){b.cancelUpload(c);return false}}};
...
"px"}if(this.height>0||this.opacity>0){var b=this;this.setTimer(setTimeout(function(){b.disappear()},30))}else{this.fileProgressWrapper.style.display="none";this.setTimer(null)}};