...
//	console.debug('fileQueued');
	try {
		...
		//$('.count-files').html('Выбрать файлы: '+document.fu.totalFileSize+' ('+ $.filesizeformat(document.fu.allSize)+')');
...
//	console.debug('fileQueueError');
	try {
		if (errorCode === SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED) {
			alert($.sprintf("Вы выбрали слишком много файлов.\nМожно выбрать не более %s файлов.", message));
			return;
		}
		...
		case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
			progress.setStatus("Размер файла превышает максимально допустимый сервером.");
			this.debug("Error Code: File too big, File name: " + file.name + ", File size: " + file.size + ", Message: " + message);
			break;
		case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
			progress.setStatus("Невозможно загрузить файл с нулевым размером.");
	...
	try {
		if (numFilesSelected > 0) {
			...
			if ( limit != -100 && limit < FileInfo.fileSize ) {
				$('.error-view').text($.sprintf('У вас осталось %s доступного места на диске. Не все фотографии будут загружены.', $.filesizeformat(limit)));
...
//	console.debug('uploadProgress', bytesLoaded, bytesTotal);	
	try {
		...
		if ( percent == 100 ) {
			progress.setStatus("Загрузка... Создание эскизов...");
		...
		$('#divStatus').text($.filesizeformat(size) +" из "+ $.filesizeformat(FileInfo.fileSize));
...
//	console.debug('uploadSuccess', serverData);
	try {
		...
		if ( serverData.match(/Allowed memory/i) ) {
			var progress = new FileProgress(file, this.customSettings.progressTarget);
			progress.setError();
			progress.setStatus($.sprintf("Недостаточно памяти для выполнения операции (memory_limit=%s)", memory_limit));
			progress.toggleCancel(false);
			FileInfo.errorAdd();
		}
	...
	try {
		...
		case SWFUpload.UPLOAD_ERROR.HTTP_ERROR:
			if ( message == '500' || errorCode == '500' ) {
				progress.setStatus("Фотография не загружена на сервер. Вероятно, нет прав для загрузки или недостаточно места на диске.: Фотография не загружена на сервер. Вероятно, нет прав для загрузки или недостаточно места на диске.");
			}				
			if ( message == '401'  ) {
		...
		case SWFUpload.UPLOAD_ERROR.UPLOAD_FAILED:
			progress.setStatus("Файл не был загружен.");
			this.debug("Error Code: Upload Failed, File name: " + file.name + ", File size: " + file.size + ", Message: " + message);
			break;
		case SWFUpload.UPLOAD_ERROR.IO_ERROR:
			progress.setStatus("Ошибка доступа к серверу (IO)");
			this.debug("Error Code: IO Error, File name: " + file.name + ", Message: " + message);
			break;
		case SWFUpload.UPLOAD_ERROR.SECURITY_ERROR:
			progress.setStatus("Ошибка загрузки файла (security error).");
			this.debug("Error Code: Security Error, File name: " + file.name + ", Message: " + message);
			break;
		case SWFUpload.UPLOAD_ERROR.UPLOAD_LIMIT_EXCEEDED:
			progress.setStatus("Превышено ограничение на загрузку.");
			this.debug("Error Code: Upload Limit Exceeded, File name: " + file.name + ", File size: " + file.size + ", Message: " + message);
			break;
		case SWFUpload.UPLOAD_ERROR.FILE_VALIDATION_FAILED:
			progress.setStatus("Файл пропущен.");
		...
		case SWFUpload.UPLOAD_ERROR.UPLOAD_STOPPED:
			progress.setStatus("Загрузка остановлена");
			break;
		default:
			progress.setStatus("Ошибка создания эксиза (возможная причина: недостаточно памяти).");
			this.debug("Error Code: " + errorCode + ", File name: " + file.name + ", File size: " + file.size + ", Message: " + message);
			break;
		}
	} catch (ex) {
