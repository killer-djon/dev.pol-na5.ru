...
//    	$quota->addDiskUsageRecord(User::getId(), PDApplication::$APP_NAME, filesize($filePath) );
    ...
    static function createThumbBySize($SIZE, $newOriginalImagePath, $newThumbImagePath)
    {
    	...
    	if ( PDImageFSModel::isImagickSetting() ) {
    		...
    		if ( file_exists($newThumbImagePath) ) {
		        ...
		        self::writeIm_($im, $newThumbImagePath);
		    	$im->destroy();
			}
		    ...
		    if ( $isW ) 
    	        ...
    	        $image->thumbnailImage(null, $SIZE);      
            if ($isSharpen) {
            	list($amount, $radius, $threshold) = explode(self::$sharpenGd);  		        
    ...
    static function writeIm_(&$im, $path) {
    	$im->setImageCompression(imagick::COMPRESSION_JPEG);
		$im->setImageCompressionQuality(IMAGICK_COMPRESSION);		
    	try {
	        $im->writeImage( $path );
		}
    ...
    public function writeIm(&$im, $path) {
    	$im->setImageCompression(imagick::COMPRESSION_JPEG);
    	
    	if ( defined('IMAGICK_COMPRESSION') )
			$im->setImageCompressionQuality(IMAGICK_COMPRESSION);
		$im->writeImage( $path );
    }
    ...
    public function createThumbToImage($newOriginalImagePath, $out, $isImagick, $isSharpen) 
    {
    	$newThumbImagePath = $out;
		...
		if ( $isImagick ) {
		    ...
		    if ( $isW ) {		        
		        ...
		        if ( $W > SIZE_970 ) {
                	...
                	$this->writeIm($image, self::prepareImagePath($newThumbImagePath, SIZE_970));	                
		        }
		        ...
		        if ( $W > SIZE_750 ) {
                	...
                	$this->writeIm($image, self::prepareImagePath($newThumbImagePath, SIZE_750));	                
		        }
		        ...
		        if ( $W > 512 ) {
                	...
                	$this->writeIm($image, self::prepareImagePath($newThumbImagePath, 512));	                
		        }
		        ...
		        if ( $W > 256 ) {
                	...
                	$this->writeIm($image, self::prepareImagePath($newThumbImagePath, 256));	                
		        }
	            ...
	            if ( $W >= 144 && $H >= 144 ) {
                	...
                	$this->writeIm($canvas, self::prepareImagePath($newThumbImagePath, 144));	                
	            }
	            else if ( $W >= 144 && $H < 144 ){
	            	if ( ($H * 144)/$W < 2 ) {
		        		$w1 = (2 * $W)/$H;
	            ...
	            if ( $W >= 96 && $H >= 96 ) {
                	...
                	$this->writeIm($canvas, self::prepareImagePath($newThumbImagePath, 96));	                
	            }
		        else if ( $W >= 96 && $H < 96 ){
	            	if ( ($H * 96)/$W < 2 ) {
		        		$w2 = (2 * $W)/$H;
		        ...
		        if ( $H > SIZE_970 ) {
    		        ...
    		        $image->thumbnailImage(null, SIZE_970);
                	$this->writeIm($image, self::prepareImagePath($newThumbImagePath, 970));	                
		        }
		        ...
		        if ( $H > SIZE_750 ) {
                	...
                	$this->writeIm($image, self::prepareImagePath($newThumbImagePath, 750));	                
		        }
	        ...
	        if ($isSharpen) {
    		    ...
    		    if ( file_exists(self::prepareImagePath($newThumbImagePath, SIZE_970)) ) {
        	        ...
        	        $im->sharpenImage(self::$sharpen[SIZE_970]['radius'], self::$sharpen[SIZE_970]['sigma']);
                	$this->writeIm($im, self::prepareImagePath($newThumbImagePath, SIZE_970));	                
                    $im->destroy();
    		    }
    		    if ( file_exists(self::prepareImagePath($newThumbImagePath, SIZE_750)) ) {
        	        ...
        	        $im->sharpenImage(self::$sharpen[SIZE_750]['radius'], self::$sharpen[SIZE_750]['sigma']);
                	$this->writeIm($im, self::prepareImagePath($newThumbImagePath, SIZE_750));	                
                    $im->destroy();
    		    }
    		    ...
    		    if ( file_exists(self::prepareImagePath($newThumbImagePath, 512)) ) {
                	...
                	$this->writeIm($im, self::prepareImagePath($newThumbImagePath, 512));	                
                    $im->destroy();
    		    }
                ...
                if ( file_exists(self::prepareImagePath($newThumbImagePath, 256)) ) {
                	...
                	$this->writeIm($im, self::prepareImagePath($newThumbImagePath, 256));	                
                    $im->destroy();
                }
                ...
                if ( file_exists(self::prepareImagePath($newThumbImagePath, 144)) ) {
                	...
                	$this->writeIm($im, self::prepareImagePath($newThumbImagePath, 144));	                
                    $im->destroy();
                }
                ...
                if ( file_exists(self::prepareImagePath($newThumbImagePath, 96)) ) {
                	...
                	$this->writeIm($im, self::prepareImagePath($newThumbImagePath, 96));	                
                    $im->destroy(); 
                }
		    ...
		    if ( $isW ) {
    		    if ( $W > SIZE_970 ) {
    		        $image->thumbnailImage(SIZE_970, null);      
	    		    if ($isSharpen) {  		        
		            	list($amount, $radius, $threshold) = self::$sharpenGd;  		        
						$image->unsharpMask($amount, $radius, $threshold);
	    			}
        ...
        foreach ( $_FILES["Filedata"]["tmp_name"] as $key => $val ) {
            ...
            if (is_uploaded_file($file["tmp_name"])) {
                ...
                try {
                    ...
                    if ( Wbs::isHosted() && $info[0]*$info[1] > 20000000 )
                    	continue;
                }
                catch (Exception $e) {
                    continue;
...
//    		$img = imagecreatefromjpeg($imagePath);
			...
			$canva = WBSImageUtilsGd::unsharpMaskStatic($canva, 80, 0.5, 3);
    		imagejpeg($canva, $this->getPublicDataAttachPath($albumId).'/thumb.jpg' );
...
//    		$img = imagecreatefromjpeg($imagePath);
			...
			$canva = WBSImageUtilsGd::unsharpMaskStatic($canva, 80, 0.5, 3);
			imagejpeg($canva, $this->getPublicDataAttachPath($albumId).'/thumbFront.jpg' );
    	}
    }
