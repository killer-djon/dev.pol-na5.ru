...
//				}
		...
		$dailySent = $model->prepare($sql)->query()->fetchField('MMS_COUNT');
		if (!$dailySent) {
			$dailySent = 0;
		}
		...
		while(($pointer < count($send_to)) || ($list_resource && ($contact_info = $list_resource->fetchAssoc()))) {
			...
			//
			if(strtoupper($message['MMM_APP_ID']) == 'MM' && MM_DAILY_SEND_LIMIT && ($pointer > (MM_DAILY_SEND_LIMIT - $dailySent))) {
			...
			if(!$forceError && !$errorStr) {
				...
				if($host) {
					...
					$header = array(
						...
						'X-Mailer' => 'WebAsyst [http://www.] MailMaster (s)',
				...
				if(!$host || !$connect)
				{
					...
					$header = array(
						...
						'From'     => $from_encoded,
						'Date'     => date('r'),
						'X-Mailer' => 'WebAsyst [http://www.] MailMaster',
						'X-Originating-IP' => '['.$_SERVER['REMOTE_ADDR'].']',
						'X-Priority' => $message['MMM_PRIORITY'],
			...
			$dailySent++;
			if(strtoupper($message['MMM_APP_ID']) == 'MM') {
				// increment count of sent messages into DB
				$sql = "REPLACE INTO MMSENT SET MMS_DATE='".date('Y-m-d')."', MMS_COUNT=i:count";
				$model->prepare($sql)->exec(array('count' => $dailySent));
			}
