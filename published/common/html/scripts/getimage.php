...
$mid = preg_replace('/\.+/', '.', $_GET['msg']);
$mid = preg_replace('/[^0-9a-z@\.\-~_]/i', '', $mid); // possible value is "some@email.box~uniq_message_id" (for inline images)

$path = $_SERVER['DOCUMENT_ROOT'].str_replace(
	'/common/html/scripts/getimage.php',
	'/data/' . $user . '/attachments/mm/images/'.$mid.'/'.basename($fileName),
	$_SERVER['SCRIPT_NAME']
);
