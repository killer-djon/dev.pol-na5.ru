<?php
	...
	class SOAP_QP_Server {
		...
		function qp_publictree( $book_id, $U_ID, $PASSWORD )
		{
			...
			foreach ($folders as $cKey => $cFolder) {
				convertEncoding($cFolder->QPF_NAME);
				convertEncoding($cFolder->QPF_TITLE);
				convertEncoding($cFolder->NAME);
				$folders[$cKey] = $cFolder;
			}
		...
		function qp_publicpage( $book_id, $page_id, $U_ID, $PASSWORD )
		{
			...
			if ( PEAR::isError( $pageData ) ) {
				...
				$f = @fopen("/tmp/support.log", "w");
				if ($f) {
				    fwrite($f, $pageData['QPF_CONTENT']);
				    fclose($f);
				}
				convertEncoding($pageData["QPF_NAME"]);
				convertEncoding($pageData["QPF_TITLE"]);
				convertEncoding($pageData["QPF_CONTENT"]);
      ...
      $res = qp_searchPages( true, iconv("WINDOWS-1251", "UTF-8", base64_decode( $search_string )), $QPB_ID, $DB_KEY, $loc_str[LANG_ENG], $textBookID=null, $entryProcessor = null, $charsAround = 50, $numEntrance = 2 );
			if ( PEAR::isError( $res ) )
				return array(
					new SOAP_Value('error', 'int', 1 ),
					new SOAP_Value('errorCode', 'int', SOAPROBOT_ERR_QUERYEXECUTING ),
			...
			{
				convertEncoding($value->QPF_NAME);
				convertEncoding($value->QPF_TITLE);
				convertEncoding($value->TEXT);
				$value->QPF_CONTENT = "";
				$res[$key] = $value;
			}
