...
function fillSendForm(id)
{
	...
	if(parent.editorIsLoaded) {
		...
		ge('messageData[MMM_CONTENT]').innerHTML = 'Загрузка...';
	}
...
function fillEditorContent()
{
	...
	if(parent.currentAction == 'reply' || parent.currentAction == 'reply_all')
		xinha_editors[_editor_textarea].setHTML(
			...
			'----- Оригинальное сообщение -----<br>' +
			'<b>От кого:</b> ' + stripAddress(parent.msgParams['from']) + '<br>' +
			'<b>Кому:</b> ' + stripAddress(parent.msgParams['to']) + '<br>\n' +
			'<b>Дата:</b> ' + stripAddress(parent.msgParams['date']) + '<br>\n' +
			'<b>Тема:</b> ' + parent.msgParams['subject'] + '<br><br>' +
...
function doSendMessage(showSendingBar)
{
	...
	if(obj.selectedIndex == -1 || !obj.options[obj.selectedIndex].value.match(re))
		alert('Неправильный адрес отправителя');
	else if(toDraft == 0 &&
		...
		alert('Неправильный адрес получателя');
	else
	{
		if(toDraft == 0)
		{
			if(showSendingBar || !ge('messageData[MMM_LISTS]').value)
				ge('sendPgBarLabel').innerHTML = 'Отправка...';
			else
				ge('sendPgBarLabel').innerHTML = 'Подготовка списка получателей...';
		}
		hideMenu('SendSubContacts');
