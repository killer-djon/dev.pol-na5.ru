<?php /* Smarty version 2.6.26, created on 2014-02-24 12:51:28
...
")'>Аккаунт</a> |
			<?php endif; ?>
			<a href="javascript:void(0)" onClick='return openLink(null, "help")'>Помощь</a> |
			<a href="<?php echo $this->_tpl_vars['url']['published']; ?>
AA/html/scripts/logout.php">Выход</a>
...
AA/html/scripts/confirm_info.php");'>Что это значит?</a>
				</div>
			<?php endif; ?>
			<?php if ($this->_tpl_vars['needBillingAlert']): ?>
				<div class="message">
					...
					<?php if ($this->_tpl_vars['controlPanelScreen']): ?>
						<a href="javascript:void(0)" onClick='return openLink(null, "<?php echo $this->_tpl_vars['controlPanelScreen']->AppId; ?>
", "<?php echo $this->_tpl_vars['url']['published']; ?>
AA/html/scripts/change_plan.php?exceed=period");'>Продлить аккаунт</a>
					<?php endif; ?>
				</div>
			<?php endif; ?>
		</div>
...
"); hideViewSelector()' href="javascript:void(0)"><?php echo $this->_tpl_vars['myAccountScreen']->Name; ?>
				...
				<?php endif; ?>
				<br />
				<a id="change-password" href="#">Изменить пароль</a>
				<div id="div-change-password" style="float:right; width: 252px; display:none">
					Введите новый пароль: <input class="password1" type="password" /><br />
					Подтвердите новый пароль: <input class="password2" type="password" /><br />
			...
			<div style="margin-top: 5px">
				<div style="float: left; width: 50%">
					<span class="label">Положение меню: </span>
				
					<table class="menupos">
						<tr>
							<td colspan="3"><input onclick="changeMenuPos(this.value)" id="radio_topmenu" value="topmenu" name="menupos" type="radio"><label for="radio_topmenu">Наверху</label></td>
						</tr>
						<tr>
							<td width="30"><input onclick="changeMenuPos(this.value)" id="radio_leftmenu" value="leftmenu" name="menupos" type="radio"><label for="radio_leftmenu"><BR>Слева</label></td>
							<td width="50">&nbsp;</td>
							<td width="30"><input onclick="changeMenuPos(this.value)" id="radio_rightmenu" value="rightmenu" name="menupos" type="radio"><label for="radio_rightmenu"><BR>Справа</label></td>
						</tr>
						<tr>
							<td colspan="3">
								<input onclick="changeMenuPos(this.value)" id="radio_bottommenu" value="bottommenu" name="menupos" type="radio"><label for="radio_bottommenu">Внизу</label>
							</td>
						</tr>
					</table>
				</div>		
				<div style="float: left; width: 49%;">
					<div style="margin-left: 20px">
						...
						<ul class="clearfix menutype">
							<li><input onClick="changeMenuType(this.value)" name="menutype" id="radio_iconslabels" value="iconslabels" type="radio"><label for="radio_iconslabels">Значки и названия</label></li>
							<li><input onClick="changeMenuType(this.value)" name="menutype" id="radio_onlyicons" value="onlyicons" type="radio"><label for="radio_onlyicons">Только значки</label></li>
							<li><input onClick="changeMenuType(this.value)" name="menutype" id="radio_onlylabels" value="onlylabels" type="radio"><label for="radio_onlylabels">Только названия</label></li>
						</ul>
					</div>
				</div>
			</div>
			<div style="float: right">
				<a href="javascript:void(0)" onClick="hideViewSelector()">Закрыть</a>
			</div>
		</div>
	</div>
...
img/fullscreen.gif" />Вернуться</a>
    ...
    document.appsData = {
        "blank": {url: "AA/html/scripts/blank.php", name: "Добро пожаловать"},
        "help": {url: "common/html/scripts/help.php", name: "Помощь"}
...
");			
	<?php endif; ?>
		var passwords_error = "Пароли не совпадают.";
	</script>
</body>
</html>