...
var ProgressManager = {
	...
	'hideProgress' : function() {
		if (ProgressManager.progressValue){ProgressManager.progressValue.style.display = 'none';}
		if (ProgressManager.progressBar){ProgressManager.progressBar.style.display = 'none';}
	},
	...
	'showProgress' : function() {
		if (ProgressManager.progressValue){ProgressManager.progressValue.style.display = 'block';}
		if (ProgressManager.progressBar){ProgressManager.progressBar.style.display = 'block';}
	},
	'onProgress' : function(progress, description)// intVal
	{
		try {
			...
			if (ProgressManager.progressStripe){ProgressManager.progressStripe.style.left = style;}
			if (ProgressManager.progressValue){ProgressManager.progressValue.innerHTML = value;}
	...
	'onComplete' : function()// ANOTHER
	{
		...
		setTimeout(function() {
					if (ProgressManager.form){ProgressManager.form.submit();}
				}, 1000);
	},
	'onGetState' : function() {
		try {
			if (xmlReq.readyState == 4) {// 4 = "loaded"
				if (xmlReq.status == 200) {// 200 = OK
					...
					if (ProgressManager.lookup){setTimeout(ProgressManager.getState, 800);}
				}
			}
